...
export default [
   {
      // Vendored build output, the node test suite, and the eval sets' run
      // packages -- none of them this page's own browser code. A run package
      // under `evals/*/runs/` is generated by
      // `skills/eval-loop/scripts/build_run_package.py` and committed as the
      // record of a run, so its `public/app.js` is build output that happens to
      // sit inside this example; holding it to this page's rules checks the
      // generator's style through a copy nobody edits here.
      //
      // Written `**/`-prefixed on purpose: flat-config ignores resolve against
      // the working directory, not this file, so `public/vendor/**` silently
      // matches nothing when eslint runs from the repo root, which is exactly
      // how the root `lint:examples` script runs it.
      ignores: [
         "**/public/vendor/**",
