...
Everything runs on local DuckDB Parquet — **no credentials required**.
...
| `storefront.malloy` | The model — `order_items` fact joined to `customers` and `products`, with reusable measures and `# dashboard` views. |
...
- **Sources:** `order_items` (fact) with `join_one` to `customers` and `products`.
...
- **Chart views:** `by_category` / `margin_by_category` / `top_brands` / `by_status`
  ...
  `sales_by_year` (`# bar_chart`), `sales_by_state` (`# shape_map`).
