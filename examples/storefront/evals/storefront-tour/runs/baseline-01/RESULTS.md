...
What survived, and why the numbers above are still quotable: a report package
had already been built from `events.jsonl`, and the console summary was
captured. Every figure in this file was read from one of those two. Neither is
committed -- the report package is derived output, and nesting one inside
`examples/storefront` is what put `storefront` into `loadErrors` -- so this
write-up is now the whole of what remains of this arm. `runs/baseline-02/`
carries the full record that this one lost, which is why the record is
committed and the bulk is not.
