...
//
// State is the current tab plus a value per given, and it lives in the URL, in
// the same shape a dashboard uses — `?tab=regions&REGION=West`. Which means a
// filtered view of this page is a link someone can send, Back undoes a drill, and
// the given names in the query string are the same names the dashboards accept.

import { barChart, destroyChart, lineChart, monthLabels, seriesLineChart, toSeries } from "./charts.js";
...
function readUrl() {
   ...
   for (const [key, value] of params) {
      ...
      // the next read (after they land) drops anything the model does not declare.
      if (key === "tab") continue;
...
const drill = createDrill({
   hasTab,
   selfLabel: "Filter this data app",
   onNavigate(slug, given, value) {
      state.tab = slug;
      state.givens[given] = value;
      // Pushed, so Back returns to where the click came from — the Console does
      // the same for a drill that leaves the page it was on.
      writeUrl({ push: true });
      syncControls();
      render();
   },
...
function panelFor(tab) {
   ...
   for (const tile of tab.tiles) {
      ...
      body.className = tile.render === "table" ? "tile-body" : "tile-body chart-wrap";
      let canvas = null;
...
function renderTile(tile, target, result) {
   ...
   if (tile.render === "table") {
      renderTable(target.body, result, drill);
      return;
   }
   ...
   if (tile.render === "line") {
      ...
      // A bar's dimension drills exactly when the same dimension drills in a
      // table, off the same `# drill` tag in the model: the chart is another
      // rendering of the field, not a second decision about it.
      const xField = result.fields.find((field) => field.name === tile.x);
      barChart(target.canvas, {
         ...
         onPick:
            xField && drill.canDrill(xField)
               ? (index, event) =>
                    drill.click(xField, result.rows[index]?.[tile.x], event)
               : undefined,
      });
...
async function render() {
   ...
   for (const [slug, other] of panels)
      other.section.hidden = slug !== state.tab;
