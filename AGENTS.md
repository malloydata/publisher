...
- Govern access: [givens](docs/givens.md) declare runtime parameters that drive filter widgets, [row-level access](docs/row-level-access.md) decides which rows a caller sees, [`#(row_authorize)`](docs/authorize.md) decides whether a caller may query a source at all, and [discovery curation](docs/discovery-and-access.md) decides what is visible in the first place. The bundled `governed-analytics` package shows all four together.
