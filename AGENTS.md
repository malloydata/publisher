...
1. `list_environments`, then pick an environment (the bundled one is `examples`) and a package (the bundled packages are `storefront`, `governed-analytics`, and `html-data-app`).
