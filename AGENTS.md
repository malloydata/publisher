...
- `PUT …/models/dashboards/{slug}.malloy`: body `{"source": "…", "expectedHash": "<sha256 of the text you read>"}`; write a dashboard file into the package and reload it in place. The text is compiled first and refused with its problems when it does not compile; the previous text is restored if the reload does not take it; a file that changed since you read it answers 409 and nothing is merged. Only dashboard files; 403 under `frozenConfig`.
