...
For your own fast checks while authoring, use `malloy_compile`; it validates a change and returns diagnostics with no server restart. When you save a model edit and want its new sources and views queryable by name, call `malloy_reloadPackage` (or `GET /api/v0/environments/<env>/packages/<pkg>?reload=true`), which recompiles just that package from disk in place, with no restart. Publisher compiles a package on first use and serves that cached model, so a source you add afterwards is not resolvable by name until the package is reloaded. That closes the edit-and-run loop without watch mode.

Compile-check with `malloy_compile` before you reload. A reload whose models do not compile removes the package's on-disk copy under `publisher_data/` and drops the cached package; recovery is a restart with `--init`, which re-copies or re-downloads the package from `publisher.config.json`, and a package that exists only under `publisher_data/` cannot be recovered. Watch-mode symlink mounts are exempt.

Both tools read the copy under `publisher_data/<env>/<pkg>/`. For an env that is not in watch mode, that is a copy Publisher made of the configured source, so editing the original source directory does nothing until you re-copy it; edit the `publisher_data/` copy, or use watch mode to mount your source in place.
