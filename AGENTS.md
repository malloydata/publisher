...
1. `malloy_listEnvironments`, then pick an environment (the bundled one is `examples`) and a package (the bundled packages are `storefront`, `governed-analytics`, and `html-data-app`).
