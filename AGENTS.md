...
- `malloy_getContext`: discovery and grounding. Call it with as much as you know and omit the rest. No arguments lists the environments, an environment lists its packages, a package lists its sources, and a plain-English query returns the most relevant sources, views, fields, and declared joins. Each response also carries a `sources` block: the doc and the complete join list for every source behind a result, so a source's grain caveats reach you even when the source itself did not rank. An empty join list means the source declares none — write the relationship inline instead of probing for it. Use the names it returns verbatim.
