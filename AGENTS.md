...
- `malloy_getStatus`: the server's health. Its operational state, each environment's loaded packages, and every configured package that failed to load or is serving a stale model. When no package is being served at all it also returns a `setup` block saying why the directory is not set up and what to do about it, which is the one case a load error cannot cover, because nothing was attempted. This is the only MCP surface where a load failure is visible on its own, so call it before concluding a package is empty or missing, and after a model edit whose reload you did not run yourself.
...
- `GET /api/v0/status`: poll until `operationalState` is `"serving"`, then check `loadErrors` (absent when everything loaded). Re-check it after every edit-and-reload: an entry with `stale: true` names a package that is still answering, from the model it compiled before your last save. This is the REST counterpart of `malloy_getStatus` for load errors, but NOT for the `setup` block: that diagnosis is MCP-only, so over REST a directory that was never set up still reports `serving` with an empty `environments` list and no error. Working unattended, read `publisher.config.json` yourself and check that each package it declares has a `publisher.json` in its directory, which is what the block would otherwise tell you.
