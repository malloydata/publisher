...
- Measure whether an agent can actually answer questions against a model, with the `eval-loop` skill: it scores answers against verified goldens, reports which entities retrieval delivered, diagnoses each failure and who owns it, and builds a servable report. [`examples/storefront/evals/storefront-tour`](examples/storefront/evals/storefront-tour) is a worked example over the bundled `storefront` package — twelve questions, goldens derived twice each, and a written-up run. Its [README](examples/storefront/evals/storefront-tour/README.md) has the exact commands, and [RESULTS.md](examples/storefront/evals/storefront-tour/RESULTS.md) is what one run produced if you would rather read one than pay for one.
