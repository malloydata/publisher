...
- `GET …/packages/{pkg}/dashboards`, then `…/dashboards/{name}`: a package's dashboards, and one dashboard's manifest. There is no run endpoint for a dashboard: take the manifest's `path` and its `query` (or each entry of its `tiles`) and send them to `…/models/{path}/query` with `givens`, which is the same governed path every other query takes.
