...
- `malloy_getContext`: discovery and grounding. Call it with as much as you know and omit the rest. No arguments lists the environments, an environment lists its packages, a package lists its sources, and a plain-English query returns the most relevant sources, views, fields, and declared joins. Anything that returns sources answers with a `sources` array, best first, with the entities that matched nested inside each source's card; the two listing tiers (no arguments, or an environment alone) return a flat `results` array instead. Each card's `source_info.resource_id` holds `environment`, `package`, `model_path` and `source`, which are what `malloy_executeQuery` takes as `environmentName`, `packageName`, `modelPath` and `sourceName`. The card's `source_info` carries that source's doc and its complete join list, so a source's grain caveats reach you even when the source itself did not rank; an empty join list means the source declares none, so write the relationship inline instead of probing for it. It also carries `givens`, the runtime parameters you may supply on a query, and `authorize`, the gate expressions in force with the givens each one reads: a source with `authorize` denies the query unless you supply those givens. A `sourceName` with no query returns that one source's card with every entity in it. Use the names it returns verbatim.
