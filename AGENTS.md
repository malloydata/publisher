...
On startup the server creates a `.mcp.json` in the directory it was run in, naming the MCP port it bound, which is why a session started in that directory finds the `malloy_*` tools with no registration step. It does not always create one, so do not promise a user the file exists without checking: it skips a directory that already has a `.mcp.json` (left exactly as it is, unread, because it may hold other servers and their credentials), any directory inside a git working tree (a clone included, which ships its own at the repo root, but also the user's own project), the home directory, and an MCP port requested as `0`. A failed write is a fifth way to end up with no file. Each case logs the `claude mcp add` command that connects an agent anyway, so read the startup output rather than assuming, and `ls -a` if you need certainty. Use that command as printed: it is deliberately local scope, because a project `.mcp.json` outranks a user-scoped entry, so `-s user` would be silently shadowed by the very file that caused the message. The file also outlives the server, so one left from an earlier run can name a dead port. `--no-mcp-config` turns it off, and the Docker image sets `PUBLISHER_NO_MCP_CONFIG=1`.
