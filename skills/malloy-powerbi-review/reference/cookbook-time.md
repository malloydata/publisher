...
Watch the reserved words too: `date`, `month` and `days` all need backticks as
field names, and a Power BI date table usually has at least one of them.
...
the same data - see T4. MTD is the same recipe with `partition_by: fy, ym`.
...
```dax
Sales MoM = [Total Sales] - CALCULATE ( [Total Sales], DATEADD ( 'Date'[Date], -1, MONTH ) )
```
...
**Nothing in Malloy densifies sparse periods.** There is no `WITH RECURSIVE`, no
generated date range, and no equivalent of a marked date table that supplies the
missing rows. This recipe is a workaround, and its existence is the evidence behind
the upstream ask.
