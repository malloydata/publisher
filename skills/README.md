...
- The four `eval-*` skills are **shared**. They were briefly classified Publisher-specific because they named `malloy_*` tools; the correct fix was to use bare tool names, which they now do. Host-only capabilities (compile, package reload, status, trace lookup) are referred to in neutral prose rather than by tool name, per the rule below. Their loop is scrape/run, eval, diagnose, improve, checkpoint: local, agent-conducted, persisted in files and git, scored by an LLM judge, with no Python orchestrator and no remote publish step.
