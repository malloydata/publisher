...
3. **DECLARE THE GIVENS** the dashboard will filter by, in the model (usually `givens.malloy`), with
   ...
   share them.
4. **WRITE THE FILE** in `dashboards/`, following the template below.
...
- **A model-level `##` tag must be on one line.** Wrapping a long `## artifact { … }` is a compile
  error.
- **A nested block must be last on its line.** `# artifact { … givens { … } } dashboard { columns=12 }`
  reads the trailing tag as part of `givens` and the grid silently never applies. Put it on its own
  line.
...
- `docs/choosing-a-surface.md`: dashboard vs notebook vs HTML data app.
- `examples/storefront/dashboards/`: four worked dashboards covering every form.
