...
- **A required entity filter on every fact source** (`report_id`, `tenant_id`, `account_id`) means
...
- **A range or multi-value pin** (`where: availability = 'Linear'`) may look selective and not be.
...
- **Filters that remove nothing.** Very common for defensive guards (`audience_level = 1`,
  `task_name = 'the_only_task'`). They cost nothing to keep, but you must not *design around* them
  as if they were selective, and you should not present them as part of the scope reduction.
- **Filters that look selective and aren't.** A two-way categorical split (`'Linear'` vs
  `'Digital'`) is often ~50/50. Splitting a table on a 50/50 axis halves it - almost never enough.
...
combination - selectivity is not a constant. The same filter cut one dataset 115× on one
...
If the fact is **polymorphic** - one wide table where a discriminator column (`dimension_set_id`,
...
schemes look more valuable than they are - collapsing 100 buckets to ~29 is only ~3.4×, which
...
The whole point is usually orders of magnitude - 68 rows to 786M. Linear bars are useless across
...
- **`<br/>` may not render** in the host's mermaid build, silently concatenating label lines into
  things like `im_flat2.77B rows`, which reads as a typo. **Don't fight it** - put the node *name*
