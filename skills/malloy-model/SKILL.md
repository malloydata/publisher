...
- **Never `except:`/`accept:` a gate dimension's field away.** A derivation that drops it (`extend { except: authorized }`, or a plain `accept:` that just doesn't re-list it) produces a source with no gate at all, and it fails OPEN: every row serves, with no load error and no warning. Treat a gate dimension's name as a field no derivation may ever drop, the same discipline you'd apply to any other sensitive column.
...
- **The one-line and quoted-string forms are refused at load and no longer exist, with one blind spot.** `#(authorize) "<expr>"` on the `source:` line, and a file-level `##(authorize) "<expr>"` applying to every source in the file, are retired; a source may declare at most one gate dimension (no stacking two annotations as OR). Always write the annotation and the `internal dimension:` line separately, on their own lines: Malloy consumes everything after `#(authorize)` on the same line as annotation text, so a one-line rewrite compiles with no gate at all and no error. The load-time refusal only sees a string-form gate within one `import` hop of the model being loaded; further away, it loads and denies every request instead, with no compile-time hint. See your deployment's reference documentation.
