...
- **Never `except:`/`accept:` a gate dimension's field away, and never redefine its name.** A derivation that drops it (`extend { except: authorized }`, or a plain `accept:` that just doesn't re-list it) produces a source with no gate at all, and it fails OPEN: every row serves, with no load error and no warning. The same is true of `extend { except: authorized, dimension: authorized is … }` — an unannotated redefinition — which also loads clean and ungated. (A `rename:` is safe: the annotation travels with the field and the gate follows the new name.) Treat a gate dimension's name as a field no derivation may drop or redefine, the same discipline you'd apply to any other sensitive column.
- The expression may reference givens, row fields (including through a join), or both; see your deployment's `#(authorize)` reference documentation for what that means for load-time versus request-time failures. One case worth knowing while authoring: comparing a row field to an array-typed given with `=`/`!=` (`org_id = $GROUPS`) compiles and loads cleanly, then fails at query execution with a warehouse conversion error. Use `in` for an array-typed given, not `=`.
- **The one-line and quoted-string forms are refused at load and no longer exist.** `#(authorize) "<expr>"` on the `source:` line, and a file-level `##(authorize) "<expr>"` applying to every source in the file, are retired; a source may declare at most one gate dimension (no stacking two annotations as OR). Always write the annotation and the `internal dimension:` line separately, on their own lines: Malloy consumes everything after `#(authorize)` on the same line as annotation text, so a one-line rewrite compiles with no gate at all and no error. Every `.malloy` file in a package compiles at load and any failure aborts the package, so a string-form gate anywhere in the package is refused with the rewrite named. Only a declaring file *outside* the package escapes that — it loads and denies every request instead, with no compile-time hint. See your deployment's reference documentation.
