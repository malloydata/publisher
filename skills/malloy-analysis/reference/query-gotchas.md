...
- [Charts: ONE Aggregate Per View](#charts-one-aggregate-per-view)
- [Joined Fields in `order_by`: Must Alias First](#joined-fields-in-orderby-must-alias-first)
- [`having:` vs `where:`: Aggregate Filters](#having-vs-where-aggregate-filters)
- [Aggregating Joined Fields: Method Syntax](#aggregating-joined-fields-method-syntax)
- [`sum`/`avg` Need a Numeric Field](#sumavg-need-a-numeric-field)
- [Dotted Paths Must Name a Declared Join](#dotted-paths-must-name-a-declared-join)
- [`order_by:` Can Only Name an Output Column](#orderby-can-only-name-an-output-column)
- [Chart Annotation Placement](#chart-annotation-placement)
- [DRY: Define in Source, Reference in View](#dry-define-in-source-reference-in-view)
- [Time Truncation vs Extraction](#time-truncation-vs-extraction)
- [`?` Alternation: Use Commas to Combine Filters](#-alternation-use-commas-to-combine-filters)
- [Query Clauses Are Newline-Separated](#query-clauses-are-newline-separated)
- [Fields Within a Clause: Commas or Newlines, Never Semicolons](#fields-within-a-clause-commas-or-newlines-never-semicolons)
