...
| Cross-model join inside a single ad-hoc report cell | A `skill:analysis-report` cell renders against a single model, so a cross-model join there won't render. Build a published `.malloynb` (which runs against the whole notebook) for a cross-model join. |
