...
description: First steps for discovering data and running a grounded query against a Malloy Publisher deployment. Use when you do not yet know the available environments, packages, or models, or when a user asks what data they can explore.
---
...
If the `malloy_*` tools are missing, stop and load `skill:malloy-publisher-setup`. This skill assumes the tools are reachable.
...
- `malloy-modeling`: build or change a model. Validate, save, reload, and run using the edit-and-run loop in `skill:malloy-publisher-setup` so the new sources and views run by name without restarting the server.
- `malloy-analysis`: explore a package and answer data questions.
- `malloy-dashboards`: a tagged `.malloy` file in the package's `dashboards/` directory, with filter controls and drill-through, no code.
