...
If there is no Publisher workspace here at all, and the user wants to work with data of their own rather than the bundled examples, `npm create malloy-package <name>` scaffolds one: the package and a starter model, registered so the server actually serves it, plus the start script, the MCP config and these skills. Seed the starter model from a local file with `npm create malloy-package <name> -- --data ./their-file.csv` (CSV, Parquet, or Excel), keeping the `--`, which is how `npm create` passes options through; a package is just Malloy, so its model can instead query a database connection the config defines. Because it writes a `.mcp.json` that did not exist when the client connected, the user has to restart or reconnect once before these tools appear, and their client will ask them to approve the new project-scoped server the first time.
