...
|---|---|---|
| `record_type_id = 17` | segment, period, window, channel | **all** segments / periods / windows / channels, at type 17 |
| type 17 + period 1 + variant 4 | segment, cohort | all segments + cohorts at that exact pin |
| *nothing* - caller picks the record type | record type, plus everything else | you cannot narrow on record type at all |
...
```malloy
// If fact_hot were narrowed to period 1, this would return EMPTY, not slow.
run: metrics_fact -> {
  where: period_id = 4      // caller-varied, deliberately not the common case
  group_by: variant_id
...
periods and all 6 variants, then it is as big as that makes it - 100M rows instead of 10M -
...
If the key callers filter on (`customer_id`, `tenant_id`, …) lives on a *joined* source rather than as
a column on the persisted table, then filtering requires joining the lookup **first**, and the
predicate cannot push down into the fact scan. A small table can then behave like a huge one -
seconds per query where comparable tiles are sub-second.

Snapshot the join values as columns at build time:

```malloy
source: _raw_joined is conn.table('…') extend {
  join_one: customer_ref on CUSTOMER_ID = customer_ref.internal_customer_id
  join_one: segment_ref  on SEGMENT_ID  = segment_ref.internal_segment_id
}
...
source: fact is _raw_joined -> {
  select:
    *
    CUSTOMER_KEY is customer_ref.customer_id      // ← the column callers filter on
    SEGMENT_NAME is segment_ref.segment_name
}
```
