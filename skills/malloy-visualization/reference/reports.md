...
- [Before building a report](#before-building-a-report)
- [Filters are inherited from the model, don't declare them in the report](#filters-are-inherited-from-the-model-dont-declare-them-in-the-report)
- [What goes in the report](#what-goes-in-the-report)
- [Section heading](#section-heading)
- [Choosing chart types and annotations](#choosing-chart-types-and-annotations)
- [Editing an existing report](#editing-an-existing-report)
- [IMPORTANT](#important)

An ad-hoc report is a `.malloynb` notebook that combines markdown narrative with live Malloy query cells. There is no dedicated report tool: you author the notebook directly. Load ``reference/notebooks.md`` for the full `.malloynb` cell format and authoring rules; this skill covers when to build one and how to design good report content (cells, chart annotations, narrative structure).
...
Read ``reference/charts.md`` before picking visualizations: it owns chart-type selection, properties, and the placement rules for chart annotations. `skill:malloy-analysis` covers Malloy query patterns and the critical placement rules for chart-annotation tags.
...
A `# dashboard` cell composes nested views, useful for KPIs alongside a trend in a single cell. Each `nest:` is a tile; any top-level `aggregate:` measures render as KPI cards. For a fixed grid, use `# dashboard { columns=N }` with `# colspan` on each tile (see ``reference/charts.md``):
