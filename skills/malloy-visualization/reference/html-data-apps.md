...
- [What "done" means (production recipe)](#what-done-means-production-recipe)
  - [Verify before you call it done](#verify-before-you-call-it-done)
- [Minimal scaffold](#minimal-scaffold)
- [Authoring loop and publishing](#authoring-loop-and-publishing)
  - [Author locally (with live reload)](#author-locally-with-live-reload)
...
- **Modular, not one inline blob.** Split the page into modules per ``reference/html-data-app-runtime.md`` (pure formatting helpers, a chart layer, your tile/query definitions as data, a thin entry point). One source of truth for each tile's model/source/view, no parallel maps that drift.
- **Every tile handles loading, empty, and error on its own.** One failing query must not blank the page. (``reference/html-data-app-runtime.md``.)
- **Defensible numbers.** Missing ≠ zero (omit the point, don't plot a fake 0); show the latest non-null value for "current"; guard division with `nullif`; convert units explicitly. (``reference/html-data-app-runtime.md``.)
...
Build row content with `textContent`, not `innerHTML` with model values: an `innerHTML` table renders any markup a value contains. This is the HTML-output side of the don't-trust-interpolated-values rule that ``reference/html-data-app-runtime.md`` applies to Malloy query strings.
