...
- [Scope: when to use this skill vs ``reference/reports.md``](#scope-when-to-use-this-skill-vs-skillmalloy-analysis-report)
- [Important Limitations](#important-limitations)
- [Multiple Models & Cross-Model Joins](#multiple-models--cross-model-joins)
- [No Data-Specific Insights in Markdown](#no-data-specific-insights-in-markdown)
- [WRONG: will become stale when data refreshes](#wrong-will-become-stale-when-data-refreshes)
- [RIGHT: frames the question, lets the query answer it](#right-frames-the-question-lets-the-query-answer-it)
- [Cell Syntax](#cell-syntax)
- [Summary](#summary)
- [Monthly Trend](#monthly-trend)
- [Interactive Parameters (`given:`)](#interactive-parameters-given)
- [Legacy: `#(filter)` annotations](#legacy-filter-annotations)
- [Notebook Style: Iterative Analysis (Default)](#notebook-style-iterative-analysis-default)
- [How is revenue trending?](#how-is-revenue-trending)
- [What's driving the biggest changes?](#whats-driving-the-biggest-changes)
- [How does the top category break down?](#how-does-the-top-category-break-down)
- [View Refinement](#view-refinement)
- [Template](#template)
- [[First question: start broad]](#first-question-start-broad)
- [[Next question: motivated by what the first query reveals]](#next-question-motivated-by-what-the-first-query-reveals)
- [[Deeper question]](#deeper-question)
...
## Scope: when to use this skill vs ``reference/reports.md``
...
For **ad-hoc reports** generated by an analysis agent, combining queries into a quick narrative artifact, use ``reference/reports.md`` instead. Ad-hoc reports also inherit whatever the source declares.
...
> **Deprecated. Read this before copying the example below.** `#(filter)` is server-side `where:` injection, superseded by native Malloy `given:` above. Do NOT add new `#(filter)` annotations; declare runtime parameters as givens instead. `docs/givens.md` has the migration recipes. What follows is here so you can read and maintain models that already use it.

Filters are declared **once, on the model's source**. The notebook itself does not declare or list filters. Publisher reads the source's filter metadata and renders the widgets above the notebook automatically, then injects `where:` clauses server-side on every cell.

Declarations live in the `.malloy` model file, above the source, using key=value parameters. The full reference (syntax, filter types, `required` / `implicit` flags) lives with the `#(filter)` guidance in the `malloy-model` skill's Parameterizable Filters section. Short example:
...
| Cross-model join inside a single ad-hoc report cell | A ``reference/reports.md`` cell renders against a single model, so a cross-model join there won't render. Build a published `.malloynb` (which runs against the whole notebook) for a cross-model join. |
