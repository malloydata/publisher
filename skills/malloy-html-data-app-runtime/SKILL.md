...
| 403 on a query that should be allowed, when passing givens to a gated source | On a source with `#(access_filter)`, a bad given (unknown name or wrong-typed value) fails closed in the authorize check, so it looks like access denied rather than validation. Check the given names and values against the model. |
