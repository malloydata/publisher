...
- [R-01: Don't bake a fixed display scale into a measure definition](#r-01-dont-bake-a-fixed-display-scale-into-a-measure-definition)
- [R-02: `# big_value` measures should have `# label`](#r-02--bigvalue-measures-should-have--label)
- [R-03: Sparklines on `# big_value` need both the `sparkline=` reference and a matching `# hidden` nested view](#r-03-sparklines-on--bigvalue-need-both-the-sparkline-reference-and-a-matching--hidden-nested-view)
- [R-04: `# big_value` comparison deltas need `comparison_label` to pair with `comparison_field`](#r-04--bigvalue-comparison-deltas-need-comparisonlabel-to-pair-with-comparisonfield)
- [R-05: Currency codes and scale values must be valid](#r-05-currency-codes-and-scale-values-must-be-valid)
- [R-06: `# number=id` for integer columns that aren't quantities (years, IDs, zip codes, phone numbers)](#r-06--numberid-for-integer-columns-that-arent-quantities-years-ids-zip-codes-phone-numbers)
- [R-08: Don't use `# hidden` as a model-visibility mechanism](#r-08-dont-use--hidden-as-a-model-visibility-mechanism)
- [R-09: Chart-rendering tags must be on the renderer's supported list](#r-09-chart-rendering-tags-must-be-on-the-renderers-supported-list)
- [R-10: Multi-series chart syntax: flat properties + quoted strings](#r-10-multi-series-chart-syntax-flat-properties--quoted-strings)
...
- **See:** `skill:malloy-visualization` § No Fixed Scale on Measures
...
- **See:** `skill:malloy-visualization` § `# big_value` Needs `# label` on Each Measure
...
- **See:** `skill:malloy-visualization` § Sparkline Setup · `skill:malloy-visualization` § Sparklines in KPI Cards
...
- **See:** `skill:malloy-visualization` § Comparison Deltas · `skill:malloy-visualization` § KPIs with Comparison Deltas
...
- **See:** `skill:malloy-visualization` § Field Formatting Tags
...
- **Why this matters.** Malloy's default integer formatting adds thousand-separator commas. A year displays as `2,018` instead of `2018`, a zip code as `94,107` instead of `94107`, an account number as `1,234,567` instead of `1234567`. The number is *visibly wrong* on every chart and table until `# number=id` strips the formatting. Especially common with `year(ts)` extraction (`skill:malloy-analysis` § Time Truncation vs Extraction) and with any FK/PK column that surfaces on a chart axis or in a result table.
...
- **See:** `skill:malloy-visualization` § Field Formatting Tags · `skill:malloy-analysis` § Time Truncation vs Extraction (year-as-int discussion)
...
- **See:** `skill:malloy-visualization` § Utility Tags (canonical `# hidden` definition) · `malloy-model/reference/access-modifiers.md` (for the `internal:` / `private:` alternative)
...
- **See:** `skill:malloy-visualization` § Chart Types
...
- **See:** `skill:malloy-visualization` § Multi-Series Bar Charts · `skill:malloy-visualization` § Stacked Bar Charts
