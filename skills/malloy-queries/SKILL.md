...
| `unexpected '<function>', expected 'not' or 'null'` | A **timeframe word used as an alias**. `second`, `minute`, `hour`, `day`, `week`, `month`, `quarter`, `year` are reserved, so `aggregate: second is avg(x)` fails - and the error points at `avg`, not at `second`, which sends you looking at the wrong half of the line. Rename the field (`second_chance`, `secs`) or backtick it. The same applies to a COLUMN whose name is a reserved word: backtick it at the point of use. |
...
| `Unknown function '<name>'. Use '<name>!(...)' to call a SQL function directly.` | The function does not exist in Malloy (`substring` is `substr`; there is no `median`). **On a restricted or proxied connection the suggested fix is a dead end** - `median!(...)` then fails with "direct SQL function calls are not permitted", so following the error message costs you two round trips. Use the Malloy spelling, or express it another way (an ordered `limit`/offset for a median-like value); do not reach for `!(...)`. |
