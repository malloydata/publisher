...
- [Reserved Words: Backtick Them](#reserved-words-backtick-them)
- [NULL Checks: `is not null`, NOT `!= null`](#null-checks-is-not-null-not--null)
- [Date Functions vs Properties](#date-functions-vs-properties)
- [`.date` Is a Cast, Not a Truncation](#date-is-a-cast-not-a-truncation)
- [Interval Functions: `unit(start to end)`, and the unit decides the operand type](#interval-functions-unitstart-to-end-and-the-unit-decides-the-operand-type)
- [Safe Division: Always `nullif`](#safe-division-always-nullif)
- [String Columns Need Casts for Aggregates](#string-columns-need-casts-for-aggregates)
- [Boolean Columns: No Quotes](#boolean-columns-no-quotes)
- [`greatest()` / `least()` Are Null-Poisoning](#greatest--least-are-null-poisoning)
- [No Scalar Median; Raw-SQL Aggregates Don't Compile](#no-scalar-median-raw-sql-aggregates-dont-compile)
- [Field Management: `extend {}` and `include {}`, in that order](#field-management-extend--and-include--in-that-order)
  - [Default: `include {}` for documented, curated base sources](#default-include--for-documented-curated-base-sources)
  - [When a `rename:` is needed: rename first, then `include {}`](#when-a-rename-is-needed-rename-first-then-include-)
  - [`extend {}` clauses (reference)](#extend--clauses-reference)
  - [Migrating `conn.sql()` to `conn.table()` + Malloy clauses](#migrating-connsql-to-conntable--malloy-clauses)
- [Cannot Redefine Query-Based Source Columns](#cannot-redefine-query-based-source-columns)
- [Extending a Source Cannot Reuse a Name It Already Defines](#extending-a-source-cannot-reuse-a-name-it-already-defines)
- [Never Use `conn.sql()` When Malloy Has a Native Pattern](#never-use-connsql-when-malloy-has-a-native-pattern)
- [Relative Data-File Paths: Set an Absolute `workingDirectory`](#relative-data-file-paths-set-an-absolute-workingdirectory)
- [JSON Files: Read Them In Place Like CSV](#json-files-read-them-in-place-like-csv)
- [Excel Files: Read `.xlsx` In Place, Never Convert](#excel-files-read-xlsx-in-place-never-convert)
- [Duplicate Rows: Check Before Building Measures](#duplicate-rows-check-before-building-measures)
- [Mixed-Grain Joins: A Pre-Aggregated Source Ignores Your Filters](#mixed-grain-joins-a-pre-aggregated-source-ignores-your-filters)
- [Thresholds Are Decisions, Not Syntax](#thresholds-are-decisions-not-syntax)
- [`except:` Removes Fields From Namespace Entirely](#except-removes-fields-from-namespace-entirely)
- [Source Order: Define Joined Tables First](#source-order-define-joined-tables-first)
- [MUST Search Docs Before Using Unfamiliar Patterns](#must-search-docs-before-using-unfamiliar-patterns)
...
Use `include {}` whenever the source doesn't need a `rename:`. It's the only way to attach `#(doc)` tags to raw columns, and it's the canonical way to hide empty/garbage/duplicate columns (`internal:`) and sensitive ones (`private:`). See ``reference/model.md`` § Access Modifiers.
...
This is the modeling-time consequence of ignoring ``reference/scope.md``'s advice to skip pre-aggregated snapshot tables and compute fresh in Malloy instead.

## Thresholds Are Decisions, Not Syntax

Before writing a `pick` expression or filtered measure with a numeric cutoff, see ``reference/model.md`` § Key Rules: every boundary must be user-supplied, distribution-derived (query the percentiles first), or explicitly flagged as an assumption in its `#(doc)`. Never invent one silently.
