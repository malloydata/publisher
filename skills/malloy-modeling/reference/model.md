...
- [Getting Started (New Projects)](#getting-started-new-projects)
- [Prior Art Dispatch](#prior-art-dispatch)
- [Base Source Templates](#base-source-templates)
  - [Base Source (Simple Mode)](#base-source-simple-mode)
  - [Base Source (Curated Mode with Access Modifiers)](#base-source-curated-mode-with-access-modifiers)
  - [Computed Source (from Query)](#computed-source-from-query)
- [Joined Source File Template](#joined-source-file-template)
- [Base vs Joined Sources](#base-vs-joined-sources)
- [Key Rules](#key-rules)
- [Parameterizing sources with `given:` (preferred)](#parameterizing-sources-with-given-preferred)
- [Legacy: Parameterizable Filters with `#(filter)`](#legacy-parameterizable-filters-with-filter)
  - [Syntax](#syntax)
  - [Filter types](#filter-types)
  - [Example](#example)
  - [When to use `required`](#when-to-use-required)
  - [When to use `implicit`](#when-to-use-implicit)
  - [Type-aware literals](#type-aware-literals)
  - [Bypass](#bypass)
- [Access Control: Source Gating with `#(authorize)`](#access-control-source-gating-with-authorize)
- [Join Syntax](#join-syntax)
- [After Writing: Check & Review](#after-writing-check--review)
...
- **`rename:` composes with `include {}`, but only in one order**: the `extend { rename: }` must come before the `include {}`, which then names the field by its new name. Reversed, it fails with `Can't find field 'X' to set access modifier`. For a cleaner column name without a rename, `internal:` + `dimension:` is still the lighter move (mark `` `Type` `` as `internal`, add `` dimension: order_type is `Type` ``). See ``reference/modeling-gotchas.md`` § Field Management
...
- **Never write a threshold, tier boundary, or bucket cutoff you chose yourself.** Every boundary in a `pick` expression or filtered measure is user-supplied, distribution-derived (query `min`/`p25`/`p50`/`p75`/`p95` first and show the evidence; see ``reference/define.md`` § Data-driven proposals), or explicitly flagged as an assumption in its `#(doc)`. A hardcoded cutoff nobody confirmed is a business decision shipped as fact.
...
- Build a notebook with interactive filters over the model (see `skill:malloy-visualization`).
