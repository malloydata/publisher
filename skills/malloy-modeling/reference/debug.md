...
- [Gotchas Checklist](#gotchas-checklist)
  - [Backtick Reserved Words](#backtick-reserved-words)
  - [Use `is not null` for NULL Checks](#use-is-not-null-for-null-checks)
  - [Call Date Functions, Don't Access as Properties](#call-date-functions-dont-access-as-properties)
  - [Use `having:` for Aggregate Filters](#use-having-for-aggregate-filters)
  - [Cast Strings Before Aggregating](#cast-strings-before-aggregating)
  - [Use Boolean Literals Without Quotes](#use-boolean-literals-without-quotes)
  - [Alias Joined Fields Before Using in order_by](#alias-joined-fields-before-using-in-orderby)
  - [Use Method Syntax for Joined Aggregates](#use-method-syntax-for-joined-aggregates)
  - [Define Lookup Tables First (or Use Imports)](#define-lookup-tables-first-or-use-imports)
  - [Use `nullif` for Division](#use-nullif-for-division)
- [Cross-File Errors](#cross-file-errors)
...
| `Can't find field 'X' to set access modifier` | An `include {}` sits before the `extend { rename: }`. Rename first, then `include {}` naming the field by its new name (see ``reference/modeling-gotchas.md`` § Field Management) |
| `IO Error: No files found that match the pattern "data/x.csv"` | Data-file path, not the model. Relative `duckdb.table()` paths resolve against the DuckDB `workingDirectory`; Publisher sets it to the package root, but a relative `workingDirectory` in `malloy-config.json` resolves against the process cwd. Make it absolute (see ``reference/modeling-gotchas.md`` § Relative Data-File Paths). The "not defined" errors under it are cascade, not real |
