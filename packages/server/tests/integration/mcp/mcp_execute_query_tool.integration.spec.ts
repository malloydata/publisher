...
describe("MCP Tool Handlers (E2E Integration)", () => {
   ...
   describe("malloy_executeQuery Tool", () => {
      ...
      // transport contract instead: a closed client rejects further tools (SDK:
      // "Not connected"; wording may also mention closed/cancelled).
      it("should reject malloy_executeQuery after the MCP client is closed", async () => {
         ...
         await expect(
            ...
            message: expect.stringMatching(/not connected|cancel|closed/i),
         });
      });
