...
describe("hydrateIncrementalKeysCte", () => {
   ...
   it("rewrites the empty CTE body to select the primary key from the target", () => {
      const sql =
         "WITH __credible_incremental_keys AS (SELECT NULL AS item_name WHERE false) " +
      ...
      expect(out).toContain(
         'WITH __credible_incremental_keys AS ( SELECT "item_name" FROM "ds"."t" )',
      );
   ...
   it("balances nested parens: matches the CTE's closing paren, not the first", () => {
      ...
      const sql =
         "WITH __credible_incremental_keys AS (SELECT x FROM (SELECT 1 AS x) z WHERE false) " +
         "SELECT * FROM src";
      const out = hydrateIncrementalKeysCte(sql, '"t"', "id", "duckdb");
      expect(out).toBe(
         'WITH __credible_incremental_keys AS ( SELECT "id" FROM "t" ) SELECT * FROM src',
      );
   });

   it("quotes the primary key for a backtick dialect (BigQuery)", () => {
      const sql =
         "WITH __credible_incremental_keys AS (SELECT 1 WHERE false) SELECT * FROM src";
      const out = hydrateIncrementalKeysCte(
         sql,
         "`ds`.`t`",
         "order_id",
      ...
      expect(out).toContain(
         "WITH __credible_incremental_keys AS ( SELECT `order_id` FROM `ds`.`t` )",
      );
   });
});
...
describe("buildOneSource incremental dispatch", () => {
   ...
   it("hydrates the self-reference CTE against the live target before MERGE", async () => {
      ...
      // __credible_incremental_keys CTE is rewritten to read the target, so the
      ...
      expect(merge).toContain(
         'WITH __credible_incremental_keys AS ( SELECT "item_name" FROM "class_v1" )',
      );
   });
});
