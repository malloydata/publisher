...
export function readGivenControlSpec(
   ...
   if (suggest) {
      ...
      // All three documented forms name their options with a `query` or a
      // `source`; `dimension` only picks a column out of one. So a block
      // carrying `dimension` alone names nothing to run, and emitting it would
      // hand a client an unfillable dropdown where omitting it falls back to a
      // plain input.
      if (parsed.query !== undefined || parsed.source !== undefined) {
         spec.suggest = parsed;
      }
   }
