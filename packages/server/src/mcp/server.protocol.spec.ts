...
describe("MCP server over the MCP protocol (in-memory)", () => {
   ...
   it("states the getContext response contract the payload actually ships", async () => {
      ...
      // snake_case throughout since the response converged on the Credible
      // platform's GetContextResponse: one shape, one parser, one mental
      // model across a local server and the hosted one.
      for (const term of [
         "entity_type",
         "sources",
         "source_info",
