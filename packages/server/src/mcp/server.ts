...
Start with malloy_getContext. Call it with no arguments to list the environments (each with its packages), with an environment to list its packages, with a package to list its sources, and with a package plus a plain-English question to get the sources, views, and fields most relevant to it. Use the names it returns verbatim and do not guess. Then run a query with malloy_executeQuery. To change a model: validate the edit with malloy_compile, save it, then call malloy_reloadPackage so the new sources and views become queryable by name without restarting the server. ${RELOAD_FAILURE_IS_SAFE} After every model edit, call malloy_reloadPackage before querying: the server serves the model compiled at the last load, not your files, and a watch-mode save that fails to compile is otherwise silent. An empty malloy_getContext result does not mean the package is empty; check malloy_getStatus for load errors and stale packages before concluding there is no data. If malloy_getStatus returns a setup key, no package is being served and the directory itself is not set up: follow setup.nextAction rather than retrying.
