...
- "append" (default): append source to modelPath. Use for NEW definitions; existing definitions report "Cannot redefine". Positions refer to the concatenated file. The source is checked against the model's published surface, so it may not declare its own data roots: import, connection.table(...), connection.sql(...) and raw-SQL functions are refused here. It also may not declare a given: (referencing the model's own givens as $NAME is fine) or set a ##! compiler flag. The text must stand alone as top-level Malloy: a bare view:/dimension:/measure:, or anything that only parses as a continuation of the model's last statement, is refused rather than compiled — wrap a view body in a top-level query:, or a field in a throwaway source: check is <source> extend { … }. Use scope "file" or "package" to validate a model that declares those.
