...
/**
 * Why a directory that was never set up needs its own diagnosis: nothing was
 * attempted, so there is no load error to report, and the server looks
 * perfectly healthy with an empty environment list.
 */
...
export type UnservedPackage = {
    ...
    * declares none. Every string this type carries is echoed as DECLARED, never
    * substituted: the location because it is the text a human edits in the
    * file, and the names because substituting them would publish the value of
    * whatever variable they reference (see the note on `interpolate`).
    * Substitution is used only to decide which path to probe on disk.
    */
   location?: string;
   /**
    ...
    *
    * Absent for THREE reasons, so its absence does NOT by itself mean
    * "remote": a remote location; a local location that could not be
    * resolved; and a local location that interpolates a variable, where the
    * resolved path is withheld because it would embed that variable's value.
    * Read `location` to tell them apart.
    */
   sourcePath?: string;
   /**
    * What sourcePath points at. Present whenever the location resolved, which
    * INCLUDES the case where sourcePath itself is withheld because the
    * location interpolates a variable: the kind is derived from the resolved
    * path but discloses nothing, so do not treat it as implying sourcePath is
    * present. A boolean would collapse "points at a file" and "points at
    * nothing", which need opposite fixes and can both appear in one config.
    */
...
export type WorkspaceSetup = {
   ...
   /** True when configuredConnections was capped and is not the full list. */
   configuredConnectionsTruncated?: boolean;
...
/**
 * Publisher serves notebooks as well as models, and the bundled skills tell an
 * agent to author them, so a workspace whose only work is a `.malloynb` is a
 * normal shape rather than an exotic one.
 */
function isModelFile(name: string): boolean {
   return (
      name.endsWith(MODEL_FILE_SUFFIX) || name.endsWith(NOTEBOOK_FILE_SUFFIX)
   );
}
 ...
 *
 * Symlinks are skipped at both depths, which takes `withFileTypes` on both
 * readdir calls: a Dirent for a symlink reports neither isFile nor
 * isDirectory. Descending a link would read a tree the server root does not
 * own.
 *
 * Sorted before the cap, because readdir order is filesystem-dependent and
 * capping the raw order would make WHICH files survive vary by platform.
 */
   ...
   for (const entry of entries) {
      if (entry.isFile() && isModelFile(entry.name)) {
         ...
         for (const child of children) {
            if (child.isFile() && isModelFile(child.name)) {
               found.push(`${entry.name}/${child.name}`);
            }
         }
      }
   }
   return found.sort().slice(0, MAX_LISTED);
}

/**
 * Substitutes `${VAR}` the way the loader does, and is applied to ONE thing:
 * the package location, so the path probed on disk is the path the loader
 * resolved.
 *
 * It is deliberately NOT applied to the environment, connection or package
 * NAME, and that restriction is a security boundary rather than a style
 * choice. A name is an arbitrary config string, so it can be written as
 * `${SOME_VAR}`; substituting it would put the VALUE of that variable into an
 * unauthenticated response. That is a real exposure rather than a theoretical
 * one, because this block is emitted only when nothing is being served, so the
 * failed environment is absent from `environments` and, in the unset-variable
 * case, there are no `loadErrors` either: nothing else has published that name,
 * so this response would be the sole discloser. Verified by probe: with a
 * connection named `${TOKEN}`, substituting produced the token's value in
 * `configuredConnections`.
 *
 * The cost is that a name written as `${VAR}` is shown as written and will not
 * match the substituted form in `loadErrors`. That is the safe direction: a
 * name an agent cannot join costs it one lookup, while a substituted secret
 * cannot be taken back. Every name that is NOT interpolated, which is nearly
 * all of them, is unaffected either way.
 *
 * Returns the raw text when substitution fails, because a diagnostic that
 * throws away a location is worse than one that shows the un-interpolated
 * form. (After the whole-config check above, substitution here cannot
 * actually throw; the fallback is belt and braces.)
 */
...
/**
 * The POSITION of a JSON syntax error, and nothing else.
 *
 * V8 builds its "Unexpected token" message from a window of the RAW SOURCE
 * around the fault, so echoing `error.message` would put a slice of
 * publisher.config.json into an unauthenticated response, and that file is
 * where a Postgres password or a Snowflake private key is written literally.
 * Verified on node v24: a trailing comma after a connection block produced
 * `Unexpected token ']', ..."ret!" },\n ]\n}" is not valid JSON`, carrying the
 * tail of the secret next to it. JavaScriptCore does not include source, so
 * this never reproduces under Bun and would not have shown up in these tests;
 * the published bin runs under Node.
 *
 * A line and column is the part an author can act on and cannot carry content.
 */
export function jsonErrorPosition(error: unknown): string {
   const message = error instanceof Error ? error.message : "";
   const match = message.match(
      /at position \d+(?: \(line \d+ column \d+\))?|line \d+ column \d+/,
   );
 ...
 * `repository.listEnvironments()` is non-empty. Keyed on whether environments
 * exist, NOT on whether `publisher_data/` does, since `initialize()` creates
 * that directory unconditionally. `--init` is destructive and the advice says
 * so: it removes `publisher_data/` and reloads from the config alone, so
 * anything created over the REST API and absent from the config is lost.
 * Pinned by environment_store_config_precedence.spec.ts.
 */
...
export async function describeWorkspaceSetup(
   ...
   // "No server root to diagnose" and "the diagnosis faulted" are different
   // events, and only the second deserves the caller's warn.
   ...
   if (configFile === null) {
      ...
      return {
         problem: hasModels
            ? `No ${PUBLISHER_CONFIG_NAME} in ${resolvedRoot}, so the ${MODEL_FILE_SUFFIX} / ${NOTEBOOK_FILE_SUFFIX} files there are not part of any package and nothing is served.`
   ...
   // opposite advice.
   // Read and parse are separate, because reporting a read failure as a syntax
   // error sends an operator to edit a file that is already correct. The
   // common one is a config baked into an image root-owned while the server
   // runs as a non-root uid: perfectly valid JSON, EACCES on open.
   let raw: string;
   try {
      ...
      return {
         problem: `${configFile} could not be READ${code ? ` (${code})` : ""}, so no environments were configured and nothing is served. The file's contents were never parsed.`,
         nextAction: `Check the file's permissions and owner against the user this server runs as, then ${restartPhrase(hasPersistedEnvironments)}. Do not edit the JSON: it has not been read, so nothing is known to be wrong with it.`,
         serverRoot: resolvedRoot,
         configFile,
      };
   }
   ...
   try {
      ...
      const detail = jsonErrorPosition(error);
      return {
   ...
   //
   // Never states the MECHANISM: whether one unset variable rejects the whole
   // file or only its own environment is changing, so the wording keys off
   // whether load errors are present instead.
   try {
      ...
      return {
         problem: hasLoadErrors
            ? `A configuration value in ${configFile} references an environment variable that is not set (${detail}); loadErrors names which environments could not be created.`
            : `A configuration value in ${configFile} references an environment variable that is not set (${detail}), so the config was rejected before any environment was created and nothing is served.`,
         nextAction: `Set that variable in the server's environment (or remove the reference), then ${restartPhrase(hasPersistedEnvironments)}. ${
            hasLoadErrors
               ? `Read loadErrors for which environments this affected; a package whose own environment is unaffected does not need editing.`
               : `No package was attempted, so there is nothing to fix in the packages themselves.`
         }`,
         serverRoot: resolvedRoot,
         configFile,
      };
   ...
   let configuredConnectionsTruncated = false;
   let declaredPackageCount = 0;
   let missingManifestCount = 0;
   let missingDirectoryCount = 0;
   let notADirectoryCount = 0;
   let anyWithheldSourcePath = false;
   let missingLocationCount = 0;
   // Entries the walk cannot read as objects at all. Counted for the same
   // reason declaredPackageCount counts a package with an unusable location:
   // this path is STATED to run on configs the validated loader rejected, so a
   // malformed entry is the expected input here, and skipping it silently
   // makes the ratio lie and can report a file that plainly declares packages
   // as declaring none.
   ...
   for (const rawEnvironment of rawEnvironments) {
      ...
      if (Array.isArray(environment.connections)) {
         for (const connection of environment.connections) {
            ...
            if (typeof name === "string") {
               ...
               // Capped like the other two lists: a config declaring a
               // connection per tenant would otherwise emit all of them.
               if (
                  !configuredConnections.includes(resolvedName) &&
                  configuredConnections.length < MAX_LISTED
               ) {
                  configuredConnections.push(resolvedName);
               } else if (!configuredConnections.includes(resolvedName)) {
                  configuredConnectionsTruncated = true;
               }
            }
         }
      }
      ...
      for (const rawPackage of Array.isArray(environment.packages)
         ...
         declaredPackageCount++;
         const packageName =
            typeof pkg.name === "string" ? pkg.name : "(unnamed)";
         ...
         // already succeeded above, so this cannot throw here. The substituted
         // form is used to PROBE the disk and is never emitted when it differs
         // from the declared text, because the difference is the value of an
         // environment variable and this response is unauthenticated.
         const location = interpolate(declaredLocation);
         const locationWasInterpolated = location !== declaredLocation;
         ...
         if (sourcePath !== undefined && locationWasInterpolated) {
            anyWithheldSourcePath = true;
         }
         if (unservedPackages.length < MAX_LISTED) {
            unservedPackages.push({
               environment: environmentName,
               ...
               // Withheld when interpolation changed the text: the resolved
               // path would embed the variable's value. The KIND and manifest
               // facts below are derived from it and disclose nothing.
               ...(sourcePath !== undefined &&
                  !locationWasInterpolated && { sourcePath }),
   ...
   if (declaredPackageCount === 0 && unreadableEntryCount > 0) {
      // Ahead of "declares no packages", which is a dead end for a config whose
      // environments are strings or an object map: there is no environment
      // object to add a package to.
      problem = `${configFile} has ${unreadableEntryCount} entr${unreadableEntryCount === 1 ? "y" : "ies"} that are not objects, so no package could be read and nothing is served.`;
      nextAction = `Check the shape of ${configFile}: "environments" must be an ARRAY OF OBJECTS, each with a "name" and a "packages" array of objects, each of those with "name" and "location". A list of bare strings, or an object map keyed by name, reads as no packages at all. Then ${restartPhrase(hasPersistedEnvironments)}.`;
   } else if (declaredPackageCount === 0) {
      ...
      nextAction = `Create ${PACKAGE_MANIFEST_NAME} (minimally {"name": "<package>"}) in the directory of each package below that reports hasManifest false${
         anyWithheldSourcePath
            ? `, which is sourcePath where one is shown and, where it is absent, the directory the location resolves to once its variable is expanded`
            : `, shown as sourcePath`
      }, then ${restartPhrase(hasPersistedEnvironments)}. Edit that directory, NOT ${hasLoadErrors ? `the ${PUBLISHER_DATA_DIR} path named in loadErrors` : `any path under ${PUBLISHER_DATA_DIR}`}: that is the server's own copy.`;
   } else {
      ...
      nextAction = `${hasLoadErrors ? "Read loadErrors and fix" : "Check"} each package at its sourcePath below, then ${restartPhrase(hasPersistedEnvironments)}. An entry with no sourcePath has no local path to show: read its location, which is either a remote location Publisher fetches (so the failure is a download or access problem), a local path this server could not resolve, or a location that interpolates an environment variable, whose resolved path is deliberately not echoed. If the failure is an unresolved name before \`.table(\`, it is a connection. ${connectionAdvice}`;
   }
   ...
   return {
      ...
      ...(configuredConnectionsTruncated && {
         configuredConnectionsTruncated: true,
      }),
   };
}
