...
export class DuckDBRepository implements ResourceRepository {
  ...
  async getProjects(): Promise<Project[]> {
    const rows = await this.db.all<any>('SELECT * FROM projects ORDER BY name');
    return rows.map(this.mapToProject);
  }

  async getProject(id: string): Promise<Project | null> {
    const row = await this.db.get<any>('SELECT * FROM projects WHERE id = ?', [id]);
  ...
  async createProject(project: Omit<Project, 'id' | 'createdAt' | 'updatedAt'>): Promise<Project> {
    ...
    try {
      ...
      if (err.message?.includes('UNIQUE') || err.message?.includes('Constraint')) {
        const existing = await this.db.get<any>('SELECT * FROM projects WHERE name = ?', [project.name]);
        if (existing) {
  ...
  async getPackage(id: string): Promise<Package | null> {
    const row = await this.db.get<any>('SELECT * FROM packages WHERE id = ?', [id]);
  ...
  async getConnection(id: string): Promise<Connection | null> {
    const row = await this.db.get<any>('SELECT * FROM connections WHERE id = ?', [id]);
