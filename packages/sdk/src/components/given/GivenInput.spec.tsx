...
describe("GivenInput: number", () => {
   ...
   it("still offers a revert when the value is not a number", () => {
      ...
      // The box reads empty and cannot be made to read otherwise: the HTML
      // value-sanitization algorithm blanks a non-numeric value on
      // input[type=number]. Verified in real Chromium, where it yields "";
      // note happy-dom does NOT sanitize and reports "12abc", so do not assert
      // on this value here; it would pass in test and be false in a browser.
      // The revert is therefore the user's only route out, and before this it
      // was hidden too, because it keyed off the rendered string being
