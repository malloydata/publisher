...
export interface UseDrillOptions {
    ...
    */
   onSelf?: (given: string, value: string) => void;
   /**
...
export function useDrill({
   ...
   const go = useCallback(
      (intent: DrillIntent, destination: string, event?: MouseEvent) => {
         if (destination === DRILL_SELF) {
            onSelf?.(intent.given, intent.value);
            return;
         }
         onNavigate?.(
            {
   ...
   const onClick = useCallback(
      (payload: DrillClickPayload) => {
         ...
         const destinations = honorable(intent.to);
   ...
   const canDrill = useCallback(
      (field: DrillField) => honorable(drillDestinations(field)).length > 0,
      [honorable],
   );
