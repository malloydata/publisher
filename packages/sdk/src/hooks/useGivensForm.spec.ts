 ...
 *
 * NOTE FOR SLICE 7 of #935, which replaces this hook with `useGivensState`:
 * these tests describe behaviours a replacement has to preserve, not an API
 * contract worth keeping. Rewrite them alongside the hook. The one that
 * matters most is the last: the re-render test guards a defect that was live
 * in this hook, and the replacement must not reintroduce it.
 */
