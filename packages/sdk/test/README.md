...
`tsconfig.json` excludes test files, in both the `.spec.` and `.test.`
spellings and both extensions. That config drives the declaration build, so
anything left in it is emitted as a stray `.d.ts` into `dist/`. The `.tsx`
globs are not implied by the `.ts` ones.

Excluded is not unchecked. `tsconfig.spec.json` typechecks exactly the set the
build config excludes, and the root `typecheck:sdk` runs it, so a type error in
a spec fails the gate. That matters more here than it looks: `bun test` strips
types without checking them, and eslint in this package is not type-aware, so
without that config a broken spec would pass everything.

If you add a test file whose name matches neither spelling, it will be missed
by both configs. Add the glob to both, and check `dist/` afterwards.

## One thing happy-dom will let you get wrong

happy-dom does not implement the HTML value-sanitization algorithm, so
`<input type="number">` will hold `"12abc"` there. Real browsers discard it and
leave the field empty (checked against Chromium). A test asserting on the
displayed value of a number input can therefore pass here and be false in
production. Assert on the behaviour around it instead, the way the
non-numeric-value test in `GivenInput.spec.tsx` does.
