...
`execute_query`, `search_malloy_docs`, `search_database_schema`), because the prefix depends on the
host. On Publisher those are `malloy_getContext`, `malloy_executeQuery`, `malloy_searchDocs`, and
