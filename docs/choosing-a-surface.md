...
> (`.malloynb`), **dashboards** (`dashboards/*.malloy`), and **HTML data apps** (`public/`) —
> with the pros, cons, and decision rules for each.
...
| **Embedding**        | SDK `<Notebook>` for React hosts; iframe embedding is a [follow-up][embed-followup]      | SDK `<Dashboard>` for React hosts; iframe embedding is a [follow-up][embed-followup]     | `Publisher.embed` — auto-resizing iframe in any host page                                                       |
...
the same two properties on its artifact tag), and `# drill`. Nor is polish — both are listed by title
rather than filename, and the same load-time lint covers the drill tags either one fires. Pick on
the shape of the deliverable instead.
...
- Embedding into a non-React host page is a follow-up
  ...
  waiting on. React hosts have the SDK `<Notebook>` component today.
...
[malloyyo-dashboards-design.md](malloyyo-dashboards-design.md). Try
...
- Embedding into a non-React host page is still a follow-up
  ...
  `<Dashboard>` component today; `Publisher.embed` cannot usefully target a dashboard route yet,
  so an HTML data app remains the surface with the complete embedding story.

**Choose it when** the deliverable is a _view you return to_ — business health, a funnel, a
team's KPIs — filtered live by whoever's looking. If the reader scans rather than reads, it's a
dashboard.
...
3. **Does it need custom design, branding, or interactions beyond renderer tags?** → **HTML data
   app**.
4. **Is the host a React application?** Use the SDK ([react-data-apps.md](react-data-apps.md))
   — Malloy's renderer, notebooks, and dashboards as React components, `<Dashboard>` included.
