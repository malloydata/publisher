...
- `list_packages`: what the server serves — every environment with its packages — which is where the names for `scopes` come from. It is also the only surface on which a package that _failed to load_ is visible: a plain listing omits it, which reads as "does not exist" rather than "is broken".
...
Two `source_info` fields govern querying the source, and both are omitted when it has neither. `givens` names the model-level runtime parameters you may supply on `execute_query`, each with its type and declared default. `authorize` lists the `#(access_filter)` gate expressions in force, with the givens each one reads.
...
Each entity carries a `data_type` where it has one, so you know whether a field can be filtered as a string or summed. `aliases` names other spellings of that same field inside its own source, collapsed into one row. Pass `include_code` to get each entity's definition as `code`: a field's Malloy expression, or a view's definition as written. It is off by default, because a source's docs should say what a field means and a definition is long, and pinning `entity_name` in the scope turns it on by itself, since a caller who has narrowed to one entity is asking what it is. A field with no `code` under that flag is a physical column rather than a derived one.

The two enumerations are the same shape, carrying the same `joins`, with `ranking: "prominence"` in place of a score. A `source` target with no `search_text` lists the package's sources with no entities under them. Adding `source` to the scope, with field targets that carry no `search_text` either, returns that source's card with every entity of those types nested in it, so `limit` there buys fields rather than being spent on a source row you already had. One card per model path that resolves the source, not one card outright: a source is queryable at its own file and at every file importing it, and each of those is its own `resource_id`. Add `model_path` to the scope to narrow to one, and `entity_name` to narrow to a single entity within it. Note that the kinds in play come only from the targets you write: a scope alone enumerates nothing. That is for enumerating a source you have already chosen; it is not a second step a question needs, because a search call already returns each matching source with its fields nested.
...
A semantic response also carries two numbers for reading a thin result: `below_cutoff_count`, how many entities scored below the relevance floor and were dropped, and `total_entities`, how many were weighed in the first place. Always read the first against the second. Every entity in scope is either at or above the floor, and so eligible to come back as a result, or below it, and so counted — the two partition the entities your targets could return (a `measure` target weighs only measures), which is why the count means nothing on its own. The same 12 rejected is a tight match out of 400 and a total miss out of 12.

An empty `sources` is the true negative on its own: nothing cleared the floor, so the package models nothing like your question. Say the data is not there and look elsewhere; rephrasing will not help, and you will see `below_cutoff_count` equal to `total_entities`, because everything was rejected. `sources` is always present, so an empty array means exactly that and never "an older server". The case where rephrasing _does_ help is the opposite shape: you got sources back, but a large fraction of the package was rejected to find them, meaning the question is only diffusely related to what is modelled. Note that a `below_cutoff_count` of 0 alongside no sources cannot occur — zero rejected means every entity was above the floor, which would have produced results — so never wait for that combination as a signal.
...
`aliases` reports one kind of redundancy the model contains: other spellings of that same field inside its own source, collapsed into one row. A model that renames a physical column without hiding the original leaves both in the schema, and `dimension: site is SITE` is one column indexed twice. The fold is decided by the compiled expression — a field whose whole definition is a reference to a sibling of the same source IS that sibling — so a derivation that merely happens to look like a rename is never folded, and neither is a reference that traverses a join to reach a different column. Hiding the raw column at the model level with Malloy's `include { internal: ... }` is better than relying on this: retrieval never sees a non-public field at all.

Nothing is folded ACROSS sources. Two sources exposing a same-named field are two different numbers — `in_store_sales.sales` and `online_sales.sales`, or a source and a filtered extension of it — so each is returned under its own card. Choose between them by reading the `docs` on each card, which is where the grain rule or the `where:` that makes the numbers differ is written; do not assume the first is the right one. A same-named field is not a shared definition, and an identical field-level `#(doc)` is not evidence of one, since an inherited field carries its parent's doc verbatim.
...
- `compile_model`: compile Malloy source against a package and return structured diagnostics (`severity`, `message`, `model`, `line`, `character`) without running a query, so an agent can validate a change while authoring instead of firing a throwaway query. Positions are 0-based; `model` is the package-relative file a diagnostic points at. A `scope` parameter says what the source means, and picking the right one is most of using this tool well:
  - `"append"` (the default): the source is appended to `modelPath` and compiled in its namespace. Right for **new** definitions and queries. Positions are relative to the model file with the source appended, so a line in your text lands after the model's own line count. The text is checked against the model's **published surface**, so it may not declare its own data roots: `import`, `connection.table(...)`, `connection.sql(...)` and the raw-SQL function forms are refused with a 400 rather than compiled. Two further refusals have nothing to do with data roots: a `given:` declaration (a fragment may **read** the model's givens as `$NAME`, it just may not declare new ones), and a `##!` compiler-flag annotation, which would otherwise let a fragment enable a language feature the curated model did not. Build on the sources the model already declares, or use `"file"` / `"package"`, where the source *is* a model file and those constructs are how it says what it reads.
  ...
  - `"package"`: runs the same worker compiler and file selection as reload over every `.malloy` and `.malloynb` file, without swapping the result into the served package. An optional `source` acts as a what-if replacement for `modelPath`, so a change that breaks an _importer_ surfaces before you save it; a path that does not exactly match an existing file is warned and treated as new. Diagnostics may name files hidden from discovery. No rows or SQL are returned, caller text remains subject to `#(access_filter)`, and a clean dry-run still needs `reload_package` after saving for the edit to serve.

  At `"append"`, the source has to stand on its own as top-level Malloy, and the two most natural ways to check an edit both fail for reasons that have nothing to do with the edit. A bare `view:` / `dimension:` / `measure:` is not a top-level statement (`no viable alternative at input 'view:'`), and resubmitting the source you are editing collides with the model's own copy (use `scope: "file"` for that). Text that parses only as a *continuation* of the model's last statement — opening with `extend {`, say — is **refused with a 400** rather than compiled: the check that keeps append text on the model's published surface reads the fragment by itself, so text that cannot be parsed by itself cannot be checked at all. To check part of a source at append scope, either send the view body as a top-level query:
  ...
  Both compile against the real source, so inherited measures resolve and `private:` fields stay hidden exactly as they would in place. An extension _adds_ to a source's namespace rather than overriding it, so a fragment reusing an existing view name reports `Cannot redefine` too; rename it for the check.

  One thing you cannot compile-check: submitted source may not contain a gate annotation on either route — `#(authorize)` or `#(access_filter)`, in any of the spellings Malloy routes — and one is rejected with a 400 rather than compiled. A source's own gate replaces the gate it would otherwise inherit from its base, so accepting one from a request would let a caller relax the author's access gate — see [authorize.md](authorize.md#security-model). There is no way to tell an author checking their own gate from a caller forging one, because the compile door has no caller identity to check. So a gate is validated by saving it and reloading: model load compiles every gate annotation in the package and reports a malformed one as a 424 naming the source. Strip the annotation if you only want to check the rest of the edit.

- `reload_package`: recompile a package from its on-disk content so a source or view added after boot becomes queryable by name, without restarting the server. This is the other half of the authoring loop: validate with `compile_model`, save, reload, then query. A reload that fails to compile leaves the package's files alone and keeps serving the previously compiled model, returning the compile errors — which is also the loop for an edit carrying a gate on either route, since those cannot go through `compile_model`.

### Health

- `get_status`: the MCP analog of `GET /api/v0/status`, reduced to what an agent needs to judge health: `operationalState`, the server `version`, each environment's loaded package names, `loadErrors`, and `emptyReason` when the server found no config and is serving nothing. It carries no connection attributes, locations, or row data. `loadErrors` is present only when something failed, and holds two different things. An entry without `stale` is a package (or, with no `package` key, a whole environment) that did not load and is therefore missing from `environments` entirely. An entry with `stale: true` is a package that IS loaded and IS answering queries, whose most recent reload failed to compile: it answers from the model compiled before that save, and its `message` is the compile error. Nothing else reports that second case, because the package reads as serving everywhere else, so call this before concluding a package is empty or missing, and after any model edit whose reload you did not run yourself (watch mode, or another process). Fixing the model and reloading clears the entry.
...
The server does not require authentication, and `execute_query` runs Malloy against the databases your models connect to, so anyone who can reach this port can read that data. The surface is not read-only either: `reload_package` mutates server state, and for a package that carries an install location a reload re-fetches it, overwriting on-disk edits. The same effects are already reachable through the equivalent REST endpoints, so this is a reason to gate the deployment rather than a reason to avoid the tools. The MCP port binds `127.0.0.1` by default, so it is not exposed on your network unless you widen it with `MCP_HOST=0.0.0.0` (or a `PUBLISHER_HOST` that is not loopback). Widening it is opt-in for a reason: MCP tools take `environmentName` and `packageName` as ordinary arguments and the discovery tools treat them as optional, so a caller who reaches the endpoint can enumerate every loaded environment. Put an authenticating gateway in front before exposing it more widely, and keep a worker reachable by more than one tenant off MCP entirely.
...
If a client cannot reach `localhost:4040`, another local process may be holding that loopback port (some editor and MCP extensions bind it). Move Publisher's MCP server to another port with `--mcp_port`, or point the client at the machine's network address. Note that a client which _can_ reach the port is not proof it reached Publisher: if the wrong process holds it, the client connects to that instead. `get_context` names the environment and packages it is actually talking to, which is the quickest way to tell.
...
When no user can reconnect your MCP client, use the REST API on port 4000: there it is the supported interface, not a fallback. This is your situation whenever you started the server yourself mid-session (an MCP client's tool list is fixed when it connects, so the Malloy tools never appear) or you run one-shot in a cloud sandbox or CI job. Discovery, query, compile, and reload are all reachable over REST. Two conveniences stay MCP-side: `search_malloy_docs` (its docs-search index lives inside the MCP server; for Malloy syntax read the bundled [`skills/`](../skills/) markdown or [docs.malloydata.dev](https://docs.malloydata.dev) instead) and `get_context`'s plain-English relevance ranking (over REST, ground from the model metadata as in step 3 below). This API is unauthenticated and the REST port binds `0.0.0.0` by default; keep it on localhost (`--host 127.0.0.1`) in a sandbox. The examples below use port 4000, but the real port is whatever `--port` or `PUBLISHER_PORT` the server was started with; `/api/v0/status` answering is the confirmation you found it.
