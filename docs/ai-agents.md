...
- `list_packages`: what the server serves — every environment with its packages — which is where the names for `scopes` come from. It is also the only surface on which a package that *failed to load* is visible: a plain listing omits it, which reads as "does not exist" rather than "is broken".
