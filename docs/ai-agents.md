...
- `malloy_compile`: compile Malloy source against a package and return structured diagnostics (`severity`, `message`, `model`, `line`, `character`) without running a query, so an agent can validate a change while authoring instead of firing a throwaway query. Positions are 0-based; `model` is the package-relative file a diagnostic points at. A `scope` parameter says what the source means, and picking the right one is most of using this tool well:
  ...
  - `"package"`: a dry-run of every `.malloy` file in the package as saved — the same reach as a reload (imports across files, hidden files included) with none of its effects on the served model. An optional `source` acts as a what-if replacement for `modelPath`, so a change that breaks an *importer* surfaces before you save it. `includeSql` is not available at this scope, and a clean dry-run still needs `malloy_reloadPackage` after saving for the edit to serve.
