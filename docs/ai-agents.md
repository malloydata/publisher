...
- `malloy_getContext`: the entry point when you do not yet know the environment, package, or model names. Prefer typed `search_targets` (`source`, `dimension`, `measure`, `view`, `dimensional_value`) plus optional `scopes` (`environment`, `package`, `model_path`, `source`) — the same contract Credible's `get_context` uses. A typed call returns a paging envelope (`ranking`, `total_available`, `returned`) of source cards with `resource_id` (pass those fields to `malloy_executeQuery` verbatim) and per-target ranked entities. Named queries come back as `view`. The older `environmentName` / `packageName` / `query` / `sourceName` adapter still works and still returns a flat `results[]`. Listing without `search_targets` or `query` is unchanged: no arguments lists environments, an environment lists packages, a package lists sources. A returned join carries its `relationship` (`one`, `many`, or `cross`) and the name of the source that declares it, so you can traverse it as `joinName.fieldName` instead of guessing at a relationship the model already defines. Alongside `results` (legacy) or source cards (typed), a retrieval response carries each source's `doc` (truncated; the model file has the full text) and its complete `joins` list — so the grain caveats and population rules a modeller writes on a source reach you whether or not the source itself ranked for your question. Because that list is complete, an empty `joins` array means the source genuinely declares none, and you should write the relationship inline rather than spending calls probing for one. The package listing (`packageName` with no query) carries the same `joins` on each source. All of this lets an agent discover what a deployment exposes and ground a query in names the model actually defines before writing it.

A semantic response also carries `belowCutoffCount`: how many entities matched the question but scored below the relevance floor and were dropped. Read a thin result with it. An empty `results` with `belowCutoffCount` of 0 is a true negative — the package models nothing related to your phrasing, so rephrasing will not help and you should say the data is not there. A large count with few results means the question is diffusely related to many entities, which is the case where rephrasing or splitting the question does help.

Two fields report redundancy the model contains. `aliases` on a result names other spellings of that same field inside its own source, collapsed into one row (a model that renames a physical column without hiding the original leaves both in the schema). `alsoIn` names other sources that hold the same concept at effectively the same score — sibling source families like `fac_*` and `fclt_*` return the same field from each, and presenting them as independent peers is what led agents to pick one arbitrarily. When you see `alsoIn`, choose between the candidates using their docs in the `sources` block; do not assume the first row is the right one. Hiding a redundant raw column at the model level with Malloy's `include { internal: ... }` is better than relying on either: retrieval never sees a non-public field at all.

Question-level retrieval is lexical (lunr/BM25) over the model's own text by default, so it matches the terms your model uses. A field named in `snake_case` (say `dep_delay`) indexes as one token, so a search for "delay" will not surface it; when a first pass comes up empty, list the package's sources or narrow with `sourceName` rather than forwarding the user's exact words. Servers started with `EMBEDDING_API_KEY` rank by embedding similarity instead, which closes that gap. The response then carries a `retrieval` field and per-entity `score`, and a `"lexical"` response from such a server adds a `retrievalReason` — of which only `indexing` is worth retrying, and it clears within seconds. See the "Semantic retrieval" section in [configuration.md](configuration.md).
