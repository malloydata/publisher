...
This response shape is shared with the Credible platform's `get_context`, deliberately, so that a harness, skill or agent written against one works against the other and a retrieval score computed on a local Publisher is comparable to one computed against the hosted service. Two differences are worth knowing if you consume both. Publisher's `entity_type` is a superset: alongside the platform's `view`, `measure` and `dimension` it returns `join` — which is the point of retrieving joins at all, since an agent that cannot see a declared join concludes the model has none — and `query` for a model-level named query. And Publisher omits the platform-only fields it has no way to mean anything by: `summary` needs an LLM summarizer, `prominence` needs query-usage telemetry, and `values` needs a dimensional value index. Omission is in-contract on both sides, so treat an absent field as absent rather than as an error.
