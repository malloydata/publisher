...
- `malloy_getContext`: the entry point when you do not yet know the environment, package, or model names. It is progressive: call it with no arguments to list the environments (each with its package names), with an environment to list its packages, with a package to list its sources, and with a package plus a plain-English question to return the sources, views, named queries, declared joins, and dimension and measure fields most relevant to it. A returned join carries its `relationship` (`one`, `many`, or `cross`) and the name of the source that declares it, so you can traverse it as `joinName.fieldName` instead of guessing at a relationship the model already defines. A retrieval response is a `sources` array, best source first, and each entry nests the entities that matched inside it. Its `source_info` holds a structured `resource_id` (`environment`, `package`, `model_path`, `source`), the source's `docs` and a `one_line_summary` cut from its first sentence, and its complete `joins` list, so the grain caveats and population rules a modeller writes on a source reach you whether or not the source itself ranked for your question. Because that list is complete, an empty `joins` array means the source genuinely declares none, and you should write the relationship inline rather than spending calls probing for one. `source_info` also carries what governs querying the source: `givens` names the model-level runtime parameters you may supply on `malloy_executeQuery`, each with its type and declared default, and `authorize` lists the `#(authorize)` gate expressions in force with the givens each one reads. Read `authorize` as "this source is gated, and these are the givens to supply", never as a predicate to evaluate yourself: the list flattens gates carried in from a source's ancestors, they combine with AND rather than OR, and a gate the registry cannot attribute reports the fail-closed placeholder `"false"` that no author wrote. Both fields are omitted when the source has none. Each entity carries a `data_type` where it has one, so you know whether a field can be filtered as a string or summed. The package listing (`packageName` with no query) is the same shape, carrying the same `joins`, with `ranking: "prominence"` and no entities under each source; a drill-down (`sourceName` with no query) returns that one source's card with every entity nested in it, so `limit` there buys fields rather than being spent on a source row you already had. A source that matched on its own terms carries a `relevance` and no entity row; a source that is present only as the parent of a field that matched takes its `relevance` from that best entity, so a matched source is never reported at null. When a response was cut short, or the package is stale, a top-level `warnings` array says so, one warning per fact: a truncation warning names how many entities matched and how many came back, so raising `limit` or narrowing the question is a remedy you can act on. All of this lets an agent discover what a deployment exposes and ground a query in names the model actually defines before writing it.
