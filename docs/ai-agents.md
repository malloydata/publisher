...
The server does not require authentication, and `execute_query` runs Malloy against the databases your models connect to, so anyone who can reach this port can read that data. The surface is not read-only either: `reload_package` mutates server state, and for a package that carries an install location a reload re-fetches it, overwriting on-disk edits. The same effects are already reachable through the equivalent REST endpoints, so this is a reason to gate the deployment rather than a reason to avoid the tools. The MCP port binds `127.0.0.1` by default for that reason, so it is not exposed on your network unless you widen it with `--mcp_host` / `MCP_HOST` (or an explicit `--host`); put an authenticating gateway in front before exposing it more widely. Cross-origin browser access is opt-in too, via `MCP_CORS_ORIGINS`. Note the REST port on 4000 still binds `0.0.0.0` by default, and a package reload is reachable there — see [configuration.md](configuration.md).
...
- After editing a package's files, `GET …/packages/{pkg}?reload=true` recompiles it, the REST form of `reload_package`. A successful reload returns the package metadata; re-fetch the model to confirm the edit took. A reload that fails to compile leaves the files alone and keeps serving the previous model.
  - **A reload needs the reload secret.** Because it replaces the served model, `?reload=true` requires the `x-publisher-reload-secret` header matching the server's `PUBLISHER_RELOAD_SECRET`, and answers `403` when no secret is configured. Requesting the package *without* `?reload=true` is a plain read and needs no header. Prefer the MCP `reload_package` tool while iterating locally: it needs no secret. See [configuration.md](configuration.md).
