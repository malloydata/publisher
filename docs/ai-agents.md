...
- `malloy_getStatus`: the MCP analog of `GET /api/v0/status`, reduced to what an agent needs to judge health: `operationalState`, each environment's loaded package names, and `loadErrors`. It carries no connection attributes or row data. Locations appear in one place only: when no package is being served at all, the response gains a `setup` block naming the server root, the config file in force (`null` when the directory has none), each configured package's source path where it is safe to show one, and one concrete next action. A healthy server never returns it. `loadErrors` is present only when something failed, and holds two different things. An entry without `stale` is a package (or, with no `package` key, a whole environment) that did not load and is therefore missing from `environments` entirely. An entry with `stale: true` is a package that IS loaded and IS answering queries, whose most recent reload failed to compile: it answers from the model compiled before that save, and its `message` is the compile error. Nothing else reports that second case, because the package reads as serving everywhere else, so call this before concluding a package is empty or missing, and after any model edit whose reload you did not run yourself (watch mode, or another process). Fixing the model and reloading clears the entry.

  The `setup` block covers the case `loadErrors` cannot: a directory that was never set up. There, nothing was attempted, so there is no load error to report and the server looks perfectly healthy with an empty `environments` list. Follow `setup.nextAction`; retrying a query or a context call cannot succeed. Fix files at the package directory `setup.nextAction` names, never under `publisher_data/`, which is the server's own copy and is the path a `loadErrors` message names. `sourcePath` gives that directory when it is present; it is deliberately absent for a remote location, for one this server could not resolve, and for one whose location interpolates an environment variable, where echoing the resolved path would disclose that variable's value.
