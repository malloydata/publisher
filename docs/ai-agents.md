...
- `malloy_listEnvironments`: what the server serves — every environment with its packages — which is where the names for `scopes` come from. It is also the only surface on which a package that *failed to load* is visible: a plain listing omits it, which reads as "does not exist" rather than "is broken".
- `malloy_getContext`: the flat `environmentName` / `packageName` / `query` request that `get_context` replaces, kept registered and unchanged so callers written against it keep working. Deprecated; new work should use `get_context`.
...
`source_info.resource_id` is that source's identity: `environment`, `package`, `model_path`, and `source`. These are what you pass to `malloy_executeQuery`, which spells them `environmentName`, `packageName`, `modelPath`, and `sourceName`. The names differ between the two calls; the values do not.
...
Two `source_info` fields govern querying the source, and both are omitted when it has neither. `givens` names the model-level runtime parameters you may supply on `malloy_executeQuery`, each with its type and declared default. `authorize` lists the `#(authorize)` gate expressions in force, with the givens each one reads.
