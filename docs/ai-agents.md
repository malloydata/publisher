...
* **Query Execution Tool**: Used by the AI to get data.
    * `malloy_executeQuery`: Executes a Malloy query and returns the results in JSON format. Supports `sourceFilters` for applying [model-declared filters](filters.md) at query time.
