...
| Governance               | Access filters and user attributes through embedding                                         | Givens, row-level access and `#(row_authorize)` apply to every tile with no wiring                               | Ahead                     |
...
ahead on governance (givens, row-level access and `#(row_authorize)` apply to every
