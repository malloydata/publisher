...
| `GET  …/packages/{pkg}/models/{path}`                  | A model's compiled metadata (sources, views, givens), limited to the names the file publishes, plus `sourceText`, the file's Malloy verbatim. `404` for a file off the package's surface. `sourceText` is left out when the file declares something it does not publish; a dashboard's text is always returned.                                                                                                           |
