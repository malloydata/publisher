...
| Method & path                                          | Does                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /api/v0/status`                                   | Server lifecycle (`operationalState`) and release `version`, plus `loadErrors` for anything configured that did not load, or that is still serving an older model because its most recent reload failed to compile (`stale: true`), and `emptyReason` when no config was found.                                                                                                                                                                                                             |
...
| `GET  …/packages/{pkg}/models/{path}`                  | A model's compiled metadata (sources, views, givens), limited to the names the file publishes, plus `sourceText`, the file's Malloy verbatim. `404` for a file off the package's surface. `sourceText` is left out when the text names a source the file does not publish; a dashboard's text is always returned.                                                                                                           |
...
| `PUT …/packages/{pkg}/models/dashboards/{slug}.malloy` | Write a dashboard file into the package and reload it: compile-first, atomic, restored on a failed reload, refused when the file changed since it was read (`expectedHash`), or when a create (no `expectedHash`) finds the file already there. Gated by `frozenConfig`.                                                                                                                                                  |
