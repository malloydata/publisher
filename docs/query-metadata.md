...
A query response carries `queryCorrelationId` — the id Publisher minted for that query and attached as its `query_id` property. Over MCP the same id is `_query_id` in the `execute_query` envelope. Look the same value up on the other side:
