...
- **A model-authored extension that drops the gate dimension's field fails open, silently** (see [above](#the-entry-point-and-only-the-entry-point)) — a *request* submitting the same derivation is denied instead, though that defence is a pattern scan over the submitted text rather than a guarantee (see [above](#the-entry-point-and-only-the-entry-point)). This is new relative to the retired string form, which stayed attached to the `source:` line and so survived a field-level `except:`/`accept:` — failing closed there instead. There is no load-time warning for this shape today; a gate dimension's name must be treated as a field no derivation may ever drop.
