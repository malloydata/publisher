...
| --- | --- |
| `POST /…/query` | Gate the run-target source; deny → zero rows where the filter applied, 403 where the field could not be resolved. Skipped entirely when the request carries `x-publisher-bypass-authorize` set to the configured `PUBLISHER_BYPASS_AUTHORIZE_SECRET`; refused when no secret is configured. |
...
A `POST /…/query` request carrying the header `x-publisher-bypass-authorize` set to the deployment's configured shared secret runs with gate evaluation **skipped**. It exists for data management: an indexer or similar back-office caller is a machine identity with no givens, so a gated source returns 403 and is never scanned — an author who tags a dimension `#(index)` on a gated source otherwise gets an empty index, the tag having opted in and the gate having silently revoked it.

The secret comes from `PUBLISHER_BYPASS_AUTHORIZE_SECRET`. It is unset by default, and while it is unset every bypass request is refused, so the bypass has to be deliberately enabled by an operator before any caller can use it. See [Deploying with the authorize bypass](authorize-bypass-deployment.md).
...
- **A request can be exempted from the gate entirely** (see [Authorize bypass](#authorize-bypass-for-trusted-data-management-callers)), but only where an operator enabled it. `x-publisher-bypass-authorize` skips evaluation when it carries the value of `PUBLISHER_BYPASS_AUTHORIZE_SECRET`; that variable is unset by default, so the bypass is refused outright until a deployment configures a secret. Where one IS configured, Publisher still does not bound who may send the header, so strip it at the edge and treat the secret as a credential. Listed first here because it is the only limitation on this page that a *deployment*, not a model, has to close.
