...
> **`except:`-ing the gate's column and then `rename:`-ing a *different* column onto that exact name
> is denied, not misbound.** `source: w is locked extend { except: org_id } extend { rename: org_id
> is owner } ` compiles — there is once again a field named `org_id` for the gate's probe to resolve
> — but Publisher checks, at request time, that every filter the executed struct inherits (a graft
> like this one, a plain author `where:`, or a join's own filter) still binds to the same field the
> declaring source named, not merely to a field with the same name. A rename that swaps in a
> different column under the gated name fails that check and is refused with the same 403 the gate
> itself uses, rather than served filtered on the wrong values.
...
**Author joins are not gated, and that is the first thing to know about the word.** A lock is a
statement about who may query the source it is declared on, not about everything reachable beneath
it. An author joining a locked source into an ungated model source does **not** carry the lock
along, so the ungated source serves its rows. The author-facing rule: joining sensitive data into an
ungated source publishes it — put the lock on the source callers enter through. A join the *caller*
writes is different: it is the caller entering the joined source, so its lock is decided. See
...
- **Author joins are not gated — on either route.** A gate on a source reached only via a `join_*` the *model* declares **does not fire** — at any depth (A→B→C), aliased, across files, or as a member of a joined composite. `run: joiner -> { … }` where `joiner` joins a gated base returns rows regardless of the base's own gate. **This is the rule authors have to design around:** joining sensitive data into an ungated source publishes it. Put the gate on the source callers enter through, and use [access modifiers](https://docs.malloydata.dev/documentation/experiments/include) (`include { public: …, private: * }`) to control what an extension re-exposes.
- **Caller joins are gated like a run target.** A join written in the request's own text — an `extend { join_* }` on the run target, a query-local `join_one` inside `-> { … }`, a join inside a caller `source:`, `query:`, view or nest, a query-expression or named-query join, an inline `extend` over a model source — is the caller entering that source, so its gate applies to it:
  - an `#(authorize)` lock is decided and a caller it does not admit gets a 403 naming the caller's own alias;
  - an `#(access_filter)` is grafted into the caller join's `ON`, so rows the caller may not see come back as nulls on the join side rather than dropping the row;
  - a composite join is refused at 403 when any member carries a row-level gate, and its locks are ANDed across the composite and every member;
  - a join whose chain to model sources cannot be established is refused: 404 when the [query boundary](discovery-and-access.md) is active, 403 in a model that declares a gate, and admitted in a model with neither;
  - a join to a source the request itself declares is gated from that source's compiled definition, as a request-declared run target is.

  A join counts as the author's only when its location is in one of the model's own files, so a join the caller adds to a model `query:` it runs is still the caller's. On `/compile` this applies at `append` scope; a `file` or `package` submission is the author's own file, so its joins are author joins. Author joins inside a source the caller joins are still not walked, and a query that carries a caller-join row gate is never served from storage or a pre-aggregation. One author join is filtered alongside a caller join, in the safe direction: a join through a named query over an *imported* source, once a caller join grafts that source's filter.
...
- **A derivation that drops the column a gate reads fails CLOSED** — see [Row-level gates](#row-level-gates) above, and the request-time binding check that closes the rename-collision case there.
...
The `include { … private: * }` layer is what controls which base columns each extension can re-expose; each extension's own gate gates consumer access to that curated surface. The base's `false` lock is a defense-in-depth backstop against a direct `run: customers_raw`. `include { … private: * }` marks a field inaccessible rather than deleting it, so its name stays taken — a later `rename:` that targets the exact same name is refused by Malloy's own grammar at compile time (`Can't rename to '…', field already exists`), before the request-time binding check above ever runs. That check exists for `except:`/`accept:`, which DO free the name up, and is what denies a `rename:` onto a name THOSE freed instead of serving it misbound.
...
doesn't carry the gate, and a scalar/array mismatch that only fails at the warehouse. Those are gaps
in the specific model, not in the mechanism.
...
- **A gate does not follow an author join** (see [above](#the-entry-point-and-only-the-entry-point)). This is the limitation with the largest practical consequence: any model source that joins gated data and is itself ungated hands that data to every caller. (A join the caller writes is gated.) Treat "which sources can a caller enter through, and what does each of them reach" as part of modelling, not as something the gate handles for you.
- **`except:`-ing, projecting away, or `accept:`-not-relisting the column a gate reads fails CLOSED** (see [Row-level gates](#row-level-gates)) — the graft cannot resolve, so the affected entry point warns at load and denies every request at 403. This is the reverse of the retired dimension form's own known gap, where the equivalent derivation dropped the gate as a plain field and served every row silently. A derivation that then re-populates the exact gated name with a *different* column — `except:`-ing it and `rename:`-ing another field onto that name, an `accept:` list doing the equivalent, or a join (author-written or caller-written) reintroducing the name — is checked at request time against the field the declaring source actually named, not merely against the name, and is denied rather than served misbound; the same check covers a gate's condition inherited transitively through a dimension, and a plain author `where:` or a join's own filter, not only a graft. The author rule remains not to recycle a gated column's name — the check is the backstop, not a reason to rely on it.
...
- **The binding check's sibling-model lookup is PACKAGE-LOCAL.** A selective import (`import { derived } from "b.malloy"`) can leave the declaring source out of the served model's own `modelDef.contents` entirely; the binding check falls back to compiling the sibling file that actually declares it, but only ever looks within the SAME package. A base declared in a file outside the package directory (imported by an absolute or relative path that escapes it) has no sibling to resolve, so a selective-import shape built on it is denied rather than served — the same "cannot prove this binds correctly" default every other unresolvable case here gets.
- **A given the entry model does not surface is usually refused, not ignored.** Malloy's given-namespace merge covers only one level of `import`, so a value for a given the entry model doesn't surface reaches Malloy's own resolution and errors (`unknown given`) rather than being silently dropped and falling back to a default. The one exception is a name some reachable gate actually references: that value is forwarded to the gate and withheld from the query (`filterGivensToModelSurface`, `model.ts`), which is what lets a gate carried in from a base in another file evaluate at all. It is withheld only when the query itself reads no given of that name. When the query does read one (a `where:` on a source from a file that declares its own defaulted given of the same name), the value is forwarded and the request 400s instead of running that `where:` at its default; import the given at the entry model to fix it. Author-side implication: for a caller to supply a given the query itself reads, some source or file within one import hop of the entry model must declare (or import) it.
