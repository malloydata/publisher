...
> on every route.
>
> This is unchanged by the `append`-scope restriction below: that refuses text declaring its own
> data roots, not text naming a gated source, and `file` and `package` are not restricted at all.
