...
- **`forceRefresh` on an API build re-seeds**; a `forceRefresh` from the scheduler does not, and neither does one on an orchestrated build (see below). A scheduled fire needs to defeat skip-if-unchanged, which is not the same as asking for a full rebuild.
...
- **Do not use `forceRefresh` for a routine refresh.** On an orchestrated build it means nothing at all: it exists to defeat skip-if-unchanged, which never runs when the host supplies the instructions. It specifically does **not** mean re-seed here — reading it that way would make every control-plane refresh a full rebuild, since an incremental source's content address never moves when its data does.
- **Ask for a rebuild per source, with `BuildInstruction.reseed`.** That is the escape hatch for a boundary or a table you no longer trust, and it is per source so one source can rebuild while the rest advance by delta in the same run.
