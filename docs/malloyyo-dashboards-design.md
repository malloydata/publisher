...
| Construct                                                                                                                                                   | Meaning                                                                                                                                                                                                                                                                |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `# artifact { title="…" givens {X=f'…'} autorun=false }` on a `query:` (Malloyyo also accepts a `view:`; Publisher reads a `query:` only, see §Drift check) | Declares the dashboard. `title` falls back to the `#"` doc comment; `givens` sets per-dashboard starting control values; `autorun=false` batches filter changes behind an Apply button.                                                                                |
...
Malloyyo's composite reads `# colspan` and `# break` off each tile's result (since its 0.2.19), and
...
The port was taken from Malloyyo in early August 2026. This is what changed on their side since,
and what Publisher does about each. Malloyyo's dashboard commits after the port are: authoring
guidance (#135), image-host allowlisting (#162), the About page (#164), a `dashboard dev` proxy fix
(#172), and hosted auth and request hardening (server-only). Nothing changed in the tag grammar.
Re-run this check against `git log -- docs/creating-dashboards.md packages/mcp-engine/src/artifacts.ts
packages/cli/src/frame-runtime` in the Malloyyo repo when the version moves.

| Malloyyo                                                                                                                                                          | Publisher                                                                                                                                                                                 | Verdict                                                                                                                                                                                            |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `# artifact` may sit on a `view:` of a source the dashboard file extends (its "Other forms")                                                                      | Reads `# artifact` off a `query:` and `## artifact` at model level only. A view-hosted tag makes the file a **silent** shared include: no listing, no URL, no finding                     | Closed by a lint finding: the file is reported as producing no dashboard, naming the tile form and the query form as the fixes (`lintArtifactOnView`). Reading the view form itself is not planned |
...
| `malloy-config.json` → `malloyyo.image_hosts` allowlists `<img>` hosts for the sandboxed frame's CSP (#162)                                                       | Dashboards render in the page with no frame CSP, so `# image` fields load from any host                                                                                                   | Not applicable. `malloy-config.json` itself is still the open question below; no movement                                                                                                          |
| `filter<timestamp>` / `filter<date>` render a time-range control with presets (today, 7/30/90 days, 12 months, custom)                                            | Bare `date`/`timestamp` get a date picker; a `filter<timestamp>` gets a text box taking filter syntax                                                                                     | Matched: `filter<date>`/`filter<timestamp>` render a time-range control with the same presets and a custom day range; a single day keeps the date picker                                           |
| `range_min` + `range_max` render a two-handled range slider                                                                                                       | One-handled slider expressing `>= N`                                                                                                                                                      | Matched: the slider has two handles, writes `[lo to hi]`, and collapses to `>= lo` with the upper handle at the ceiling so an older URL still reads                                                |
| Suggestions are faceted: a control's options narrow by the other controls' draft values                                                                           | Suggest queries deliberately run without the page's other givens, because a `#(access_filter)` gate evaluated against a partial set would make the control unusable (`useSuggestOptions.ts`) | Deliberate; keep, and revisit only with a way to know which givens a suggest may take                                                                                                              |
| `useUrlState` for components (#129), `dashboard dev` behind a proxy (#172), `dashboard bundle` static sites (#117), hosted auth (Aug 19)                          | No component runtime, no CLI, no hosted app                                                                                                                                               | Not applicable                                                                                                                                                                                     |
| `@malloydata/malloy` `^0.0.431`                                                                                                                                   | `^0.0.432`                                                                                                                                                                                | Fine                                                                                                                                                                                               |
| No notebook format. Nothing in the repo reads or writes `.malloynb`; prose lives in the About page and the `#"` doc strings                                       | Serves `.malloynb` read-only through `<Notebook>`; the Workbook editor saves its own JSON to browser storage and has an open "export to .malloynb" TODO                                   | Recorded for the surface discussion in [choosing-a-surface.md](choosing-a-surface.md); not a dashboard-format matter                                                                               |
...
`givens` names a collection of _declarations_ everywhere on the API. Values keyed by given name
