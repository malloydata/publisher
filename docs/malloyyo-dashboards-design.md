...
**Status:** Shipped. Phases 1–3 (discovery and REST, the tag-only viewer, `# drill`) and phase 5
(the [`storefront` dashboards](../examples/storefront/dashboards), [dashboards.md](dashboards.md),
the skill) are in; phase 4 (custom JSX components) was built and then **cut**. Written July 2026,
revised against the implementation. Per-item status is in [Phasing](#phasing); the design sections
below describe what exists unless they say otherwise. To use the feature rather than read its
design, go to [dashboards.md](dashboards.md).
**Related:** [choosing-a-surface.md](choosing-a-surface.md) (when to reach for a dashboard over a notebook or
an HTML data app), [security-posture.md](security-posture.md) (the trust boundary the JSX cut
turns on), Malloyyo's `docs/creating-dashboards.md`, `docs/composite-dashboards.md`, and
`docs/dashboard-iframe-security.md` (the format and security posture this design adopts).
...
| Renderer grid (`# dashboard {columns}` / `# colspan` / `# break`) | `@malloydata/render` — Publisher SDK is on `^0.0.423`, exactly the version Malloyyo requires; `.malloy-dashboard` chrome is themed in `packages/sdk/src/components/RenderedResult/`                                                           | Have                                             |
...
- **Part of the SDK's public surface.** The SDK is the supported way to use Malloy's renderer,
  notebook, and dashboard types in a React data app ([react-data-apps.md](react-data-apps.md));
...
[choosing-a-surface.md](choosing-a-surface.md) sold this as an escape hatch for a _tile_ that
...
| 5. Example, docs, tests     | `dashboards/` in an example package (all four forms), a `<Dashboard>` usage in `examples/data-app` (the standalone-React proof), `docs/dashboards.md` (linking [choosing-a-surface.md](choosing-a-surface.md) for the when-to-use guidance), a dashboards agent skill, test suite                                                                                                        | Shipped. [`examples/storefront/dashboards/`](../examples/storefront/dashboards) carries all four forms plus an untagged shared include, over the same data as that package's notebook and HTML data app, so [choosing-a-surface.md](choosing-a-surface.md) can be read against something concrete; `examples/data-app`'s `PackageDashboard` is the standalone-React proof; [dashboards.md](dashboards.md) and `skills/malloy-dashboards` are in. Writing the examples surfaced two real bugs — a source-level `where:` referencing a given never reached tile introspection, so a composite over a given-scoped source showed an empty control row; and a single-query dashboard was clipped to the composite's per-tile height |
