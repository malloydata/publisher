...
| Suggestions are faceted: a control's options narrow by the other controls' draft values                                                                           | Suggest queries deliberately run without the page's other givens, because an `#(authorize)` gate evaluated against a partial set would make the control unusable (`useSuggestOptions.ts`) | Deliberate; keep, and revisit only with a way to know which givens a suggest may take                                                                                                              |
