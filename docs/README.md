...
| [row-level-access.md](row-level-access.md) | Restrict *which rows* a caller sees (given-scoped `where:` + `#(authorize)`). |
| [authorize.md](authorize.md) | Gate *who* can query a whole source with `#(authorize)`. |
| [discovery-and-access.md](discovery-and-access.md) | Control *what* is discoverable and queryable (`explores` / `queryableSources`) — the visibility axis. |
