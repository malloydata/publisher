...
| `PUBLISHER_NO_MCP_CONFIG` | `--no-mcp-config` | _unset_ | Set to `true` (or pass the flag) to stop the server writing `.mcp.json` into the working directory. By default it writes one when the directory has none, pointing at the MCP port it actually bound, so an agent launched in that directory finds this server without `claude mcp add`. An existing file is merged into rather than replaced: only the `malloy` key is set, other servers are left alone, and a file that cannot be parsed as JSON is never rewritten. The home directory is skipped. Note this is the **working** directory, not `--server_root`, because the file is for whoever opens an agent there. |
