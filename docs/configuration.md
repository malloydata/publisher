...
| `PUBLISHER_NO_MCP_CONFIG` | `--no-mcp-config` | _unset_ | Set (to anything) to stop the server creating `.mcp.json`. By default it creates one in the **working** directory, not `--server_root`, naming the MCP port it bound, so an agent launched there finds this server without `claude mcp add`. It only creates: an existing file is left untouched and unread, and the server logs the command to add itself instead. It skips git working trees and the home directory. Creating a new untracked file is the point, so expect one in that directory. |
