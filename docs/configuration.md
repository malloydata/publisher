...
| `PUBLISHER_NO_MCP_CONFIG` | `--no-mcp-config` | _unset_ | Set to `1`, `true`, `yes` or `on` to stop the server creating `.mcp.json`; `0`, `false`, `no`, `off` and empty leave it on. Compared after trimming and case-folding, and **any other value is a startup error**, like every boolean flag here, so do not quote it: an env file that passes `"1"` through with the quote characters attached will stop the server rather than disable the feature. By default it creates one in the **working** directory, not `--server_root`, naming the MCP port it bound, so an agent launched there finds this server without `claude mcp add`. It only creates, never edits: an existing file is left untouched and unread. It also skips git working trees, the home directory, the filesystem root, and any run where the port it bound is not the port it asked for (`--mcp_port 0`, or, under Bun, a non-numeric `MCP_PORT`; Node refuses to bind at all). In each of those it logs, at `info`, the `claude mcp add` command instead, at local scope, which is the scope that outranks a project `.mcp.json`. The URL it writes and prints names the address the server actually bound (the loopback literal of that family when the bind is a wildcard), rather than `localhost`, which is ambiguous across IPv4 and IPv6. That is for correctness, not isolation: any process running as you can bind the same port, and on macOS it can do so even while this server holds the wildcard. The file it writes outlives the server and is never corrected, so a stale one does not merely fail: it points an agent at whatever else holds that port, which may be another Publisher serving different data. Set in the Docker image, where no agent session can start. |
