...
| `PUBLISHER_NO_MCP_CONFIG` | `--no-mcp-config` | _unset_ | Set to `1`, `true`, or any other value to stop the server creating `.mcp.json`; `0`, `false`, `no`, `off` and empty leave it on (compared after trimming, case-folding, and stripping one pair of surrounding quotes). By default it creates one in the **working** directory, not `--server_root`, naming the MCP port it bound, so an agent launched there finds this server without `claude mcp add`. It only creates, never edits: an existing file is left untouched and unread. It also skips git working trees, the home directory, and any run where the port it bound is not the port it asked for (`--mcp_port 0`, or a non-numeric `MCP_PORT`, both of which change every run). Whenever it does not write one it logs, at `info`,  the `claude mcp add` command instead, at local scope, which is the scope that outranks a project `.mcp.json`. The URL it writes and prints names the address the server actually bound (the loopback literal of that family when the bind is a wildcard), rather than `localhost`, which is ambiguous across IPv4 and IPv6. That is for correctness, not isolation: any process running as you can bind the same port, and on macOS it can do so even while this server holds the wildcard. The file it writes outlives the server, so a stale one names a port nothing is on. Set in the Docker image, where no agent session can start. |
