...
| `PUBLISHER_MCP_TRACE` | — | `off` | Local `malloy_getContext` tracing. `off`: no persistence and no response trace metadata. `metadata`: hashes and counts only. `retrieval`: persist exact requests and ranked responses in `publisher.db`, and register `malloy_getTrace`. Exact traces never enter console logs. |
| `PUBLISHER_MCP_TRACE_RETENTION` | — | `10000` | Maximum persisted traces. Oldest unreferenced traces are evicted past this count. Traces linked from an eval run are exempt. |
| `PUBLISHER_EVAL_STORE` | — | `off` | Local evaluation workspace. When `on`, Publisher exposes `/api/v0/evals` and persists eval sets, cases, runs, events, and named checkpoints in `publisher.db`. No eval MCP tools are registered: the loop is agent-conducted (`eval-loop`: scrape/run, eval, diagnose, improve, checkpoint) and the answerer must not see gold. `--init` preserves eval tables; only `POST /api/v0/evals/reset` removes them. Restore is `POST /api/v0/evals/checkpoints/:id/restore`. |
| `PUBLISHER_DIMENSION_VALUE_INDEX` | — | `off` | Local dimensional-value index for typed `malloy_getContext`. `off`: `dimensional_value` targets return empty. `lexical`: query distinct values for dimensions tagged `#(index)` on sources with no `#(authorize)` or `#(filter)`, store them in `publisher.db`, and rank by substring. Values never leave the machine for embedding. `--init` drops the cache. |
| `PUBLISHER_DIMENSION_VALUE_CAP` | — | `500` | Maximum distinct values stored per indexed dimension. One extra row is fetched to detect truncation. Must be a positive integer. |
