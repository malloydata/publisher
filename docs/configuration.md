...
| `PUBLISHER_NO_MCP_CONFIG` | `--no-mcp-config` | _unset_ | Set to `1`, `true`, or any other value to stop the server creating `.mcp.json`; `0`, `false` and empty leave it on. By default it creates one in the **working** directory, not `--server_root`, naming the MCP port it bound, so an agent launched there finds this server without `claude mcp add`. It only creates, never edits: an existing file is left untouched and unread. It also skips git working trees and the home directory. In all three cases it logs the `claude mcp add` command instead. The file it writes outlives the server, so a stale one names a port nothing is on. |
