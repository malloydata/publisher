...
| `PUBLISHER_MAX_RESPONSE_BYTES` | — | `50000000` (50 MB) | Maximum JSON-serialized response size, for ad-hoc SQL, model queries and notebook cells. What it measures is the shape the caller asked for, so on the model-query endpoint two requests differing only in `compactJson` reach this cap at different result sizes. Over MCP it measures the compact rows the tool's envelope is built from, which is not what the agent receives: that is capped separately at 90k characters by the envelope's own budget. Streaming-capable connections (Postgres, DuckDB) enforce mid-stream and abort the driver immediately; non-streaming connections enforce post-buffer. Exceeding the cap fails with HTTP 413. Set to `0` to disable. |
