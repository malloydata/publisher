...
| `PERSIST_COLOCATED_RELAXATION_ENABLED` | — | `false` | Opt-in: allow a **colocated** `#@ persist` source to materialize despite carrying an [`#(authorize)`](authorize.md) gate, when that gate is proven row-level and attributed to the entry point. `false` (default): any authorize-gated colocated persist source is refused unconditionally (HTTP 422), proven or not. `true`: it materializes, and the gate is re-evaluated live against the artifact on every query — what goes stale between rebuilds is the row data, never the gate. Read at startup (a typo fails the boot) **and** at package load. Enable deliberately: a gated source carrying `#@ persist` is not refused at publish, so turning this on starts materializing and binding sources in packages nobody republished. See [materialization.md](materialization.md#the-freshness-contract-for-a-gated-colocated-persist-source). |
