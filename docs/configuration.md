...
| `PUBLISHER_HOST` | `--host <addr>` | `0.0.0.0` | Host binding for the REST server, and for the MCP server unless `MCP_HOST` overrides it. Set `127.0.0.1` to keep both loopback-only. |
| `MCP_HOST` | `--mcp_host <addr>` | `127.0.0.1` | Host binding for the MCP server only. Defaults to loopback because the MCP endpoint is unauthenticated and exposes every MCP tool; when `PUBLISHER_HOST` is set explicitly and this is not, the MCP server follows `PUBLISHER_HOST` so `--host` still moves both listeners together. Set `0.0.0.0` to expose MCP beyond the host, and put an authenticating gateway in front when you do. |
| `MCP_CORS_ORIGINS` | _none_ | _unset_ | Comma-separated origins allowed to read a cross-origin response from the MCP endpoint (`*` allows any). Unset means no cross-origin access: no `Access-Control-Allow-Origin` is sent, so a browser page on another origin cannot read a response. A non-browser MCP client sends no `Origin` and is unaffected. |
...
| `PUBLISHER_BYPASS_AUTHORIZE_SECRET` | _none_ | _unset_ | Shared secret a request must present in the `x-publisher-bypass-authorize` header to skip `#(authorize)` gate evaluation. Compared in constant time. While unset or blank the bypass is refused outright, so it must be deliberately enabled; treat the value as a credential and still strip the header at your edge. See [Deploying with the authorize bypass](authorize-bypass-deployment.md). |
| `PUBLISHER_RELOAD_SECRET` | _none_ | _unset_ | Shared secret a request must present in the `x-publisher-reload-secret` header to trigger a package reload via `GET /…/packages/{pkg}?reload=true` (and its legacy `/projects/…` alias). Compared in constant time. While unset or blank, `?reload=true` answers `403`; reading package metadata without `?reload=true` needs no secret, and the MCP `reload_package` tool is unaffected. |
