...
| `PREAGGREGATE_MODE` | — | `off` | Controls `#@ preaggregate`, which rolls a measure up to one or more stored grains (one annotation line each) so covered queries read the smallest pre-aggregated table that covers them instead of the base. `off`: a valid annotation is inert — nothing extra is built and every query serves as before. `build-only`: rollups are built so their cost can be measured, but queries still serve live. `on`: build **and** route, with a per-query fallback to live for anything no rollup covers. Read at startup. Note the rungs are ordered by cost rather than risk: `build-only` spends without saving, so it is a deliberate measurement step, and `off` is the resting state. Moving **down** is always safe, since every query is answerable live by construction — it costs latency and nothing else, and does not drop the built tables. Independent of validation: an annotation that could never take effect is refused at publish and at load in every mode, including `off`. |
