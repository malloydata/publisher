...
| `PUBLISHER_DUCKDB_MEMORY_LIMIT` | — | _unset_ | DuckDB `memory_limit` applied to **every** Publisher-owned DuckDB session, as a flat value (`1GB`, `512MB`) passed to DuckDB verbatim. Unset — or `off` — leaves DuckDB's own default, which is roughly 80% of the container's memory computed **per instance**. Publisher runs several instances in one process (the environment lookup funnel, a per-package sandbox, a disposable session per materialization build), and each derives that share independently, accounting for neither the resident Node/Bun baseline nor the other instances. Measured in a 3 GiB container: three instances each reported a 2.3 GiB limit — 6.9 GiB of committed budget against 3 GiB of real memory — so the process is killed by the kernel while every session still believes it is inside its budget. Flat rather than a fraction because any fraction is still computed per instance; only an absolute per-session value bounds the sum. Set it to roughly (container memory − resident baseline) ÷ sessions you expect concurrently. Too low fails a query with a DuckDB out-of-memory error and leaves the process up, which is the intended trade against losing the pod. |
| `PUBLISHER_DUCKDB_TEMP_DIRECTORY` | — | _unset_ | Where DuckDB spills, for every Publisher-owned session except a materialization build (a build always spills inside its own disposable working directory, which is unique per build and removed with it). Unset leaves DuckDB's default of `.tmp` relative to the process working directory, whose default `max_temp_directory_size` is 90% of whatever filesystem that lands on — on a container with no ephemeral-storage limit, the node's shared disk. Note that setting `PUBLISHER_DUCKDB_MEMORY_LIMIT` does not by itself create spill on the `storage=` build path: that pipeline pushes its SQL to the source warehouse and streams the result into the destination, with no join, sort or aggregation to spill. Local compute, and therefore spill, is the chained-build and serve paths. |
