...
whose target is not materialized, say — every query on that source falls back to
live, rather than the source serving from storage without the filter. Serving
fewer queries from the tier is a cost; serving the wrong rows is not a trade
worth making. For the same reason a filter is never dropped when the shape sheds
