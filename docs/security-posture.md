...
**3. Markdown in a notebook is rendered with raw HTML parsing enabled.** `markdown-to-jsx` runs
with its default `disableParsingRawHTML: false`, so raw HTML in a package's markdown becomes
JSX. React will not execute an inline `<script>` this way, and link `href`s are already scheme-
checked precisely because packages can come from untrusted git or S3 sources — so this is a
narrow surface, not an open one. Still, it is the one place a declarative artifact touches
author-controlled HTML, and it is worth either disabling raw HTML or sanitizing deliberately.

**4. Resize messages are not origin-checked.** Both the host runtime and the Console's
`DataAppViewer` validate `event.source` against the iframe's `contentWindow` but never
...
Pointed at HTML data apps instead, it would close gap 2 properly and raise the floor for the
surface that actually runs author code. It cannot be the default: an opaque origin breaks
