...
> [row-level access](row-level-access.md), and [`#(row_authorize)`](authorize.md) gates.
...
| **Source authorization**                 | A `#(row_authorize)` boolean expression over givens is grafted onto the source as a row filter, so a caller it admits nowhere gets a normal 200 with zero rows. A 403 means the gate could not be attached at all. | [Authorize](authorize.md)               |
...
There is no longer a separate "whole-source (given-only)" gate class — every `#(row_authorize)` gate is a
...
`#(row_authorize)` expression or scoped by a source-level `where:` that reads a
...
- **A `required` filter has no direct equivalent.** A given always has a value,
  ...
  that is safe to run, or by using `#(row_authorize)` where the requirement is
...
a `#(row_authorize)` denial, both surface this way.
