...
> dashboard declared entirely in Malloy tags, with no code and no build step. For _when_ to reach for
> a dashboard over a notebook or an HTML data app, read
> [choosing-a-surface.md](choosing-a-surface.md) first; this page assumes you have decided.
...
The bundled `storefront` package ships four of them. Start the server and open
`http://localhost:4000/examples/storefront/dashboards/overview`:

![The storefront Business Overview dashboard — a filter row over KPI cards, a revenue trend, a US choropleth, and a clickable category table](screenshots/storefront-dashboard-page.png)

Everything on that page comes from one tagged query in
[`examples/storefront/dashboards/overview.malloy`](../examples/storefront/dashboards/overview.malloy).
The two controls at the top are not written anywhere in the page; they are rendered from the two
`given:` declarations the query filters by. The category names in the table are clickable because
the model's `category` dimension carries a `# drill` tag.
...
Cards and tiles share one grid, so a page that lines up is a matter of four tags. All four
dashboards in `storefront` use the same recipe, which is what makes them read as one product rather
than four pages:
...
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `# artifact { title= givens{…} autorun= }` on a `query:`/`view:` | Declares the dashboard. `title` falls back to the `#"` doc comment; `givens` sets starting control values; see [Apply](#apply) |
...
- **A model-level `##` tag has to be on one line.** Wrapping a long `## artifact { … }` across lines
  is a compile error.
- **A nested block has to be last on its line.** `# artifact { autorun=false givens { … } }
dashboard { columns=12 }` reads the trailing `dashboard` tag as part of `givens`, and the grid
  quietly never applies. Put the second tag on its own line.
...
the grid stops loading.
[`examples/governed-analytics`](../examples/governed-analytics/dashboards/governed-overview.malloy)
is the worked case. A package with no `explores` (like `storefront`) has curation off, so an import
is all it needs.
...
![The Seasonality composite dashboard — three separately-run tiles under one shared control row](screenshots/storefront-dashboard-composite.png)
...
wiring. The storefront notebook's `category_performance` cell offers both of the destinations above
for exactly this reason, and says nothing about drill itself — it imports `CATEGORY` for its own
Parameters panel, which is all `to=self` needs. Both surfaces run the same
...
dimension that only exists to be grouped by is worth declaring for its own sake:
storefront's views group by `category` rather than `products.category` so the cells carry the tag,
with identical output field names and identical numbers.
...
that never appears, a click that goes nowhere:

- A `# drill { to=… }` naming a dashboard that does not exist in the package.
- A `to=self` drill whose given no model in the package declares, so the clicked value has nowhere
  to land on any surface.
- A given surfaced by a dashboard whose `suggest` names a source or query that file cannot see.
- A composite tile that does not resolve to a real view, or a `dashboard_columns` that is not a
  positive integer.
...
A file that fails to compile is still listed, with its error, rather than silently disappearing —
a broken dashboard should be visibly broken.
...
does this, and [react-data-apps.md](react-data-apps.md) covers the SDK more broadly.
...
- [choosing-a-surface.md](choosing-a-surface.md) — notebook, dashboard, or HTML data app
...
- [`examples/storefront`](../examples/storefront) — the four dashboards this page quotes
