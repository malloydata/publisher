...
- **Govern** — givens, row-level access, and `#(row_authorize)` decide who sees what; discovery curation
...
- **Decide who sees what.** [Givens](docs/givens.md) declare runtime parameters and drive filter
  widgets; [row-level access](docs/row-level-access.md) and [`#(row_authorize)`](docs/authorize.md) gate
...
- **[governed-analytics](examples/governed-analytics)** — the whole governance story in one small
  package: givens, row-level access, and `#(row_authorize)` source gates.
