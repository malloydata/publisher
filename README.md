...
| `PUBLISHER_MAX_QUERY_ROWS`                | —                                               | `100000`  | Maximum rows returned by the ad-hoc connection SQL endpoints (`/connections/.../sqlQuery` and the deprecated `queryData` GET). Forwarded to the connector as `RunSQLOptions.rowLimit`; queries that exceed the cap fail with HTTP 413 rather than serializing the response. Set to `0` to disable. A caller-supplied `rowLimit` smaller than the cap is preserved. |
