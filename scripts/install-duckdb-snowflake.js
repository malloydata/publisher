...
async function installDuckDB() {
  ...
  if (platform === 'win32') {
    ...
    printInfo('Please install DuckDB manually from https://duckdb.org/docs/installation/');
    printInfo('Or use DuckDB via npm package (already installed as dependency)');
    return;
  }
...
async function installADBCDriver() {
  ...
  if (needsDownload) {
    ...
    printInfo('Driver wheel already exists and appears valid, checking if extraction is needed...');
  }
  ...
  if (fs.existsSync(finalPath)) {
    ...
    console.log('');
    printSuccess('Installation complete! You can now use the Snowflake extension.');
  } else {
