...
wait_for_server_latest() {
  ...
  while :; do
    if answer="$(npm view @malloy-publisher/server dist-tags.latest --prefer-online 2>&1)" \
      ...
      if [ "$errors" -ge "$POLL_MAX_REGISTRY_ERRORS" ]; then
        echo "::error title=Server pin::npm stopped answering for @malloy-publisher/server dist-tags.latest (${errors} consecutive failures), so create-malloy-package was NOT dispatched. Once that reads ${NEW_VERSION}, dispatch create-malloy-package-npm.yml on main."
        echo "- \`create-malloy-package\` NOT dispatched: npm did not answer for the server's \`latest\`" >> "$GITHUB_STEP_SUMMARY"
        return 1
      fi
    fi
  ...
  echo "::error title=Server pin::@malloy-publisher/server latest still reads '${seen}', not ${NEW_VERSION}, after $((SERVER_LATEST_BUDGET_SECONDS / 60))m, so create-malloy-package was NOT dispatched: it would pin the previous server. Once \`npm view @malloy-publisher/server dist-tags.latest\` reads ${NEW_VERSION}, dispatch create-malloy-package-npm.yml on main."
  echo "- \`create-malloy-package\` NOT dispatched: the server's npm \`latest\` never read ${NEW_VERSION}" >> "$GITHUB_STEP_SUMMARY"
  return 1
}
