...
- **`useGivensForm` and `UseGivensFormResult` are removed.** Use `useGivensState`, which additionally covers URL round-tripping and Apply batching. `GivenValue` is unchanged and still exported from the package root.
...
> **This paragraph no longer describes current behaviour.** It was accurate at 0.0.201 and stopped being so when the notebook's Filters panel was removed: the notebook no longer sends `filterParams` at all. See "the notebook's controls are `given:` only" in the unreleased section at the top of this file for what a `#(filter)` model does now. The REST parameters themselves are untouched.
