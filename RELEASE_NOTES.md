...
- **Content-addressed physical tables** (`<name>__m<hash>`) so generations coexist and a rebuild is a safe pointer-flip; a plain logical-name **view** is maintained over the current generation for operator convenience. `DELETE …/materializations/{id}?dropTables=true` reclaims a storage table (destination-aware drop).
...
- **Eligibility gate (HTTP 422 / failed build):** a `storage=` source with an unbound free parameter, a given reference (a security refusal — a frozen given-filtered table would leak rows across tenants), a non-DuckDB-portable served shape, or a quoted `name=` identifier is refused. A source protected by `#(authorize)` should also not be materialized (the served shape carries no gate); that refusal lands alongside the upstream transitive-`#(authorize)` enforcement it reuses — until then, serve authorize-gated sources live.
...
- **Single-replica under `on`.** Serve bindings are per-replica, re-derived from the local materialization store on package load — the wire manifest doesn't carry them yet. Run `PERSIST_STORAGE_MODE=on` on a single replica (or with a host that supplies bindings to every replica) until multi-replica binding distribution lands; otherwise different replicas can serve frozen vs. live for the same source.
