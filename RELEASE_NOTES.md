...
- **The SPA route `/{env}/{pkg}/pages/{file}` → `/{env}/{pkg}/data-apps/{file}`.** A bookmark or shared link using the old form no longer opens the embedded viewer. It now redirects to the static-file route and answers 404 there, because `pages` is no longer a segment the app claims (see `SPA_OWNED_SEGMENTS` in `spa_fallback.ts`, which this change updates). The standalone URL (`/environments/{env}/packages/{pkg}/{file}`) is unchanged, so a link straight to the served HTML still works.
...
- Update any stored link of the form `/{env}/{pkg}/pages/{file}`.

Why no alias: carrying both spellings would mean two paths, two operationIds and two generated client methods for one listing, and every future change to it would have to be made twice. The endpoint is documented (in [docs/html-data-apps.md](docs/html-data-apps.md) and [docs/api-overview.md](docs/api-overview.md), both updated here), so this is a real break for anyone who took it up rather than a quiet one. If that trade is wrong for your deployment, say so on the PR.
