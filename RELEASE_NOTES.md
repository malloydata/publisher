...
Failures are reported *beside* `entries` rather than inside one, which is the part worth knowing if you consume a manifest. `entries` now holds only sources that BUILT, so every entry names a table that exists and a consumer resolving one needs no further check. A failure carries the `physicalTableName` the source was headed for — useful for correlating with the request, and never a table to read, because the build that would have created it is what failed.
