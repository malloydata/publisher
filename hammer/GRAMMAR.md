...
The step vocabulary. For what a scenario is _for_ — rule vs. observation, when to
...
What a section _does_ with its body depends on the body's **shape**:
...
aggregate, build it, query it — then mutate the source and query _again_, where an
...
**Requires** lists capability tokens a scenario needs _beyond_ the always-present
...
won't skip. Wiring a brand-new _warehouse_ type (BigQuery/Snowflake, which need
...
| `## SQL <label>`                                                                | ` ```sql ` block + `Expect:` table                                                                      | run raw SQL on the source warehouse, compare rows (e.g. _prove_ the source really changed)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `## Operator <conn>`                                                            | ` ```sql ` block + optional `Expect:` table                                                              | operator SQL on a persist target, **external to the publisher** — a DuckLake destination via the operator's own read-write client, any other connection via the scenario's source warehouse. Provisioning (`CREATE SCHEMA`) and asserting (`SELECT … ` + `Expect:`) both go here: the publisher's own paths are deliberately narrower (its serve attach is read-only, and its connection SQL endpoint cannot run a statement returning no rows). Use it to assert what the tier actually WROTE — which schema received a table, what it was named. `<conn>` must be one this scenario declared, or the always-present `orders_pg` / `lake`: an unknown name throws rather than falling through to the source warehouse, where a typo would run somewhere plausible and guard nothing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `## Connection <conn> (refused)`                                                | ` ```sql ` block                                                                                        | run SQL THROUGH the publisher's `sqlQuery` endpoint; `refused` asserts it's rejected — either because the name is a storage destination and so absent from the connection namespace, or because a user connection onto a lake is attached read-only. (Same header as the declaration above — the `type=` attribute picks the declaration form; a ` ```sql ` block picks this one)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `## Model [<pkg>/]<path>`                                                       | ` ```malloy ` block                                                                                     | write a package model file (re-declaring it mid-run = an edit)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `## Publish [<pkg>] (forceRefresh, reseed, sources=a[+b], async, label=X)`      | optional `expect binding: src -> conn`                                                                  | load + build the package; `forceRefresh` rebuilds even if unchanged; `reseed` sends an incremental source back to a full rebuild, discarding its `covered_through` boundary (`forceRefresh` does NOT do this); `sources=` builds only the named persist source(s) (the `sourceNames` filter; `+`-separated), leaving the rest live; `async` fires the build WITHOUT awaiting (see `## Await`) so a following step can observe it in flight                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `## Await [<label>]`                                                            | —                                                                                                       | drain an async publish (by `label`, else the oldest pending) and assert it completed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `## Build [refused] (orchestrated, pkg=P, pub=, env=, strict)`                  | `- <src> -> <physicalName> @ <dest> [(failed)]` lines + `reference: <src> [(from=<pub>)]` lines; optional `cites:` | orchestrated (caller-instructed) build: each source builds into the caller-assigned physical name (verified) at the destination; `reference:` reuses an already-built upstream, resolved BY NAME from the latest manifest (the target's, or `from=<pub>`); `refused` asserts the whole run fails. Per-source `(failed)` — this source is instructed and EXPECTED to fail, asserted against the committed manifest's `failures`; required, because a part-way failure still commits a manifest and mirrors a failed source into `entries` under the physical name it was headed for, so a name-only check reads a failure as a success. An unmarked source is asserted BUILT *and* absent from `failures`. Mutually exclusive with `refused` (a refused run commits no manifest to check)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `## Build targets [(pkg=P)]`                                                  | `Expect:` + GFM table `source \| writes [\| entity]`                                                     | assert what the compiled build plan CONTAINS: one row per persist source and the physical name each writes (the resolved `#@ persist name=`, else the source name). An `entity` column groups by content address — same label = same `sourceEntityId`, different labels must differ (the label is arbitrary). Exact set, sorted both sides. Runs nothing |
| `## Build refusals [(pkg=P)]`                                                 | `Expect:` + GFM table `source [\| tier \| reason]`                                                       | assert the plan's OTHER collection: sources compiled, EXAMINED and refused by the tier-appropriate eligibility gate. Distinct from a source the plan never contained, which is an absence and reads as "never examined" (see `host-binding-of-unplanned-source`). `source` is required; `tier` (`storage`/`colocated`/`preaggregate`) and `reason` (`given`/`free_parameter`/`authorize`) are opt-in, the way `entity` is above. An EMPTY table asserts nothing was refused — the only way to pin the absence side positively. Runs nothing. Not to be confused with `## Build refused`, which runs a build and asserts it fails |
...
| `## Manifest [<pkg>]`                                                           | `- <source> -> <table> @ <destination> [(unplanned, fallback=live)]` lines                              | author the manifest **a host would send**, and bind it — distinct from `## Bind`, which replays one the publisher produced. The interesting entries are the ones the publisher would never generate: a source it refused to build, a stale generation, a table it does not own. Each source is resolved to its `sourceEntityId` through the package build plan, and the captured schema is copied from whichever entry already describes that physical table, so a real build must have produced it first (see `host-binding-honors-row-level-access`). Per-entry attributes: `unplanned` — the build plan does not know this source (a source Malloy dropped from the plan), so the source name is used as the handle; required, because otherwise an unknown source is a typo and throws, and it is re-checked so the scenario fails if the source later becomes planned. `fallback=<live\|stale_ok\|fail>`, `asof=<iso>`, `fresh=<seconds>` — stamp the freshness fields on THIS entry; `## Bind` stamps all entries alike, so a **mixed** set is only expressible here, and mixed is the normal case for a host stamping per generation (see `runtime-fallback-reads-the-served-shape`) |
| `## Query <label> (again, refused)`                                             | ` ```malloy ` (or reuse by label) + `Expect:` table                                                     | run + compare rows; `refused` = the query MUST fail (optional `cites:`). Body keys: `givens: NAME=v; OTHER=v` supplies runtime given values; `columns: exact` asserts the column set; `servedFrom: storage` asserts which tier produced the answer (`storage` / `live_fallback`) — the only direct evidence a materialized table was read, since a correct tier and a live fallback return the same rows by design. Asserted positively, never by absence. Rejected with `refused`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `## Hook <exportName>`                                                          | —                                                                                                       | call a `hooks.ts` export, in document order (interleave with markdown steps). Receives a shared `state` object, so a tiny hook can stash a value another hook reads. **Reserved for what markdown genuinely can't express** — and reaching for one usually means a step is missing (see README, _When not to write a scenario_)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
